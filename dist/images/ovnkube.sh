...
ovn_udn_ndp_proxy_enable=${OVN_UDN_NDP_PROXY_ENABLE:=}
...
ovnkube-controller() {
  ...
  if [[ -n ${ovn_udn_ndp_proxy_enable} ]]; then
	  udn_ndp_proxy_enabled_flag="--enable-udn-ndp-proxy=${ovn_udn_ndp_proxy_enable}"
  fi
