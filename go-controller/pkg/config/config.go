...
func completeGatewayConfig(allSubnets *configSubnets, masqueradeIPs MasqueradeIP) error {
	...
	if err != nil || utilnet.IsIPv6(v4MasqueradeIP) {
		return fmt.Errorf("invalid gateway v4 masquerade subnet specified, subnet: %s: error: %v", Gateway.V4MasqueradeSubnet, err)
	}
	if err = allocateV4MasqueradeIPs(v4MasqueradeCIDR.IP, masqueradeIPs); err != nil {
	...
	if err != nil || !utilnet.IsIPv6(v6MasqueradeIP) {
		return fmt.Errorf("invalid gateway v6 masquerade subnet specified, subnet: %s: error: %v", Gateway.V6MasqueradeSubnet, err)
	}
	if err = allocateV6MasqueradeIPs(v6MasqueradeCIDR.IP, masqueradeIPs); err != nil {
