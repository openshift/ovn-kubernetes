...
func MarshalPodAnnotation(pannotations *map[string]string, podInfo *PodAnnotation, netName string) error {
	...
	podNetworks, err := unmarshalPodAnnationAllNetworks(annotations)
	if err != nil {
		return err
	}
	pa := podAnnotation{
...
func unmarshalPodAnnationAllNetworks(annotations map[string]string) (map[string]podAnnotation, error) {
