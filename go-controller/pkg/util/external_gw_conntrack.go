...
func getIPv6MacOnIface(info *ifaceWithTargetIPs) ([]net.HardwareAddr, error) {
	...
	ipsToFind := sets.NewString(info.ips...)
...
func SyncConntrackForExternalGateways(gwIPsToKeep []string, podsGetter func() ([]*kapi.Pod, error)) error {
	ipv6IPs := []string{}
	ipv4IPs := []string{}
	for _, gwIP := range gwIPsToKeep {
		if len(gwIP) > 0 {
