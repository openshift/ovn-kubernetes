...
func GetMACAddressFromARP(neighIP net.IP) (net.HardwareAddr, error) {
	klog.Infof("SURYA %v", neighIP)
	hwAddr, duration, err := arping.Ping(neighIP)
	klog.Infof("SURYA %v, %v, %v", hwAddr, duration, err)
	if err != nil {
		return nil, err
	}
	return hwAddr, nil
...
func DeleteConntrack(ip string, port int32, protocol kapi.Protocol, ipFilterType netlink.ConntrackFilterType, labels [][]byte) error {
	...
	if len(labels) > 0 {
		...
		if err := filter.AddLabel(netlink.ConntrackUnmatchLabels, labels); err != nil {
