...
func ParseNodeHostAddressesExcludeOVNManagedNetworks(node *kapi.Node) ([]string, error) {
	...
	if ovnManagedNetworks.IPv4 != "" {
		networks = remove(networks, ovnManagedNetworks.IPv4)
	}
	if ovnManagedNetworks.IPv6 != "" {
		networks = remove(networks, ovnManagedNetworks.IPv6)
	}
	return networks, nil
}
...
func remove(strs []string, candidate string) []string {
	...
	for _, idxToRemove := range idxsToRemove {
		strs = removeIndex(strs, idxToRemove)
	}
	return strs
}
