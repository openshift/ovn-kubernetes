...
const (
	...
	// maxRetryCount is the maximum number of times to retry a DNS lookup before exponential backoff starts
	maxRetryCount = 10
)
...
type dnsValue struct {
	...
	// Number of times the DNS lookup has been retried
	retryCount int
}
...
func (d *DNS) updateOne(dns string) (bool, error) {
	...
	if retry {
		...
		if res.retryCount > maxRetryCount-1 {
