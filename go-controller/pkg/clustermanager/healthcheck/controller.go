...
// latest node state and an actual health state
func transitionNodeState(old nodeState, health HealthState, on time.Time, timeout time.Duration, reset bool) nodeState {
	new := old
...
func (c *controller) reconcile(nodeName string) error {
	...
	if node == nil {
		...
		if updated {
			klog.Infof("Stopping health state monitor of node %s: node deleted", nodeName)
	...
	if !c.hasInterestedConsumers(nodeName, &names) {
		...
		updated := c.cancelProbeWithState(nodeName, UNKNOWN)
		if updated {
			klog.Infof("Stopping health state monitor of node %s: no interested consumers out of %q", nodeName, names)
	...
	if len(mgmtIPs) == 0 {
		...
		if updated {
			klog.Infof("Stopping health state monitor of node %s: node management IPs unknown", nodeName)
			c.informConsumers(nodeName)
		}
		return nil
	}
	...
	klog.Infof("Starting or updating health state monitor of node %s on IPs %s", nodeName, util.StringSlice(mgmtIPs))
...
func (c *controller) setNodeState(node string, health HealthState, on time.Time, reset bool) bool {
	...
	new := transitionNodeState(old, health, on, c.timeout, reset)
	...
	if old.health != new.health {
		klog.Infof("Health state change of node %s: from %s to %s", node, old.health, new.health)
		return true
	}
