...
func (tn testNode) Node() *corev1.Node {
	...
	if len(tn.HostCIDRs) > 0 {
		hostCIDRsJSON, _ := json.Marshal(tn.HostCIDRs)
		annotations["k8s.ovn.org/host-cidrs"] = string(hostCIDRsJSON)
	}
	return &corev1.Node{
		ObjectMeta: metav1.ObjectMeta{
			Name:        tn.Name,
			...
			vteps: []*vtepv1.VTEP{
				{
					...
					Spec:       vtepv1.VTEPSpec{CIDRs: vtepv1.DualStackCIDRs{"100.64.0.0/16"}},
				},
			},
			nodes:                []*testNode{{Name: "node", SubnetsAnnotation: "{\"cluster_udn_blue\":\"10.2.1.0/24\"}", HostCIDRs: []string{"100.64.0.1/24"}}},
			reconcile:            "ra",
			expectAcceptedStatus: metav1.ConditionTrue,
			expectFRRConfigs: []*testFRRConfig{
				{
...
`,
		...
		},
		{
			...
			nodes:                []*testNode{{Name: "node", SubnetsAnnotation: "{\"default\":\"1.1.0.0/24\"}", HostCIDRs: []string{"100.64.0.1/24"}}},
			reconcile:            "ra",
			expectAcceptedStatus: metav1.ConditionTrue,
			expectFRRConfigs: []*testFRRConfig{
				{
...
`,
		...
		},
		{
			...
			nodes:                []*testNode{{Name: "node", SubnetsAnnotation: "{\"cluster_udn_blue\":\"10.2.1.0/24\",\"cluster_udn_green\":\"10.3.1.0/24\"}", HostCIDRs: []string{"100.64.0.1/24"}}},
			reconcile:            "ra",
			expectAcceptedStatus: metav1.ConditionTrue,
			expectFRRConfigs: []*testFRRConfig{
				{
...
`,
		...
		},
		{
			...
			nodes: []*testNode{
				{Name: "node1", SubnetsAnnotation: "{\"cluster_udn_blue\":\"10.2.1.0/24\"}", HostCIDRs: []string{"100.64.0.1/24"}},
				{Name: "node2", SubnetsAnnotation: "{\"cluster_udn_blue\":\"10.2.2.0/24\"}", HostCIDRs: []string{"100.64.0.2/24"}},
			},
			reconcile:            "ra",
			expectAcceptedStatus: metav1.ConditionTrue,
			expectFRRConfigs: []*testFRRConfig{
		...
		},
		{
			...
			vteps: []*vtepv1.VTEP{
				...
				},
				{
					...
					Spec:       vtepv1.VTEPSpec{CIDRs: vtepv1.DualStackCIDRs{"200.10.0.0/16"}},
				},
			},
			nodes:                []*testNode{{Name: "node", SubnetsAnnotation: "{\"cluster_udn_blue\":\"10.2.1.0/24\",\"cluster_udn_green\":\"10.3.1.0/24\"}", HostCIDRs: []string{"100.64.0.1/24", "200.10.0.1/24"}}},
			reconcile:            "ra",
			expectAcceptedStatus: metav1.ConditionTrue,
			expectFRRConfigs: []*testFRRConfig{
				{
