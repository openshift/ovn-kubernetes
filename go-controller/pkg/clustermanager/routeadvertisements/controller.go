...
func (c *Controller) generateFRRConfigurations(ra *ratypes.RouteAdvertisements) ([]*frrtypes.FRRConfiguration, []*nadtypes.NetworkAttachmentDefinition, error) {
	...
	// Parse VTEP CIDRs and discover per-node VTEP IPs by matching host-cidrs.
	// This runs after EVPN validation so c.vtepLister (only initialized when
	// EVPN is enabled) is safe to dereference.
	if vtepNames.Len() > 0 {
		var vtepCIDRs []*net.IPNet
		for _, vtepName := range sets.List(vtepNames) {
			...
			for _, cidr := range vtep.Spec.CIDRs {
				...
				if err != nil {
					return nil, nil, fmt.Errorf("invalid CIDR %q in VTEP %q: %w", cidr, vtepName, err)
				}
				vtepCIDRs = append(vtepCIDRs, ipNet)
			}
		}
		...
		for _, node := range nodes {
			...
			if err != nil {
				...
				klog.Warningf("Skipping node %s for VTEP IP discovery: %v", node.Name, err)
				continue
			}
			for hostCIDR := range hostCIDRs {
				...
				for _, cidr := range vtepCIDRs {
					if cidr.Contains(ip) {
						...
						if utilnet.IsIPv6String(ip.String()) {
							vtepIPsByNode[node.Name].Insert(ip.String() + "/128")
						} else {
							vtepIPsByNode[node.Name].Insert(ip.String() + "/32")
						}
						break
					}
				}
...
func vtepNeedsUpdate(oldObj, newObj *vtepv1.VTEP) bool {
	...
	// mode changes will come later when managed mode is supported
	// CEL validation ensures ordering never changes
	return !reflect.DeepEqual(oldObj.Spec.CIDRs, newObj.Spec.CIDRs)
}
