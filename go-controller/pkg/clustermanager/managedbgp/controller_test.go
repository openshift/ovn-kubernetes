	...
	ginkgo.Context("Full-mesh topology", func() {
		...
		ginkgo.It("should create base FRRConfiguration with IPv4 nodes", func() {
			...
			gomega.Eventually(func() int {
				list, _ := frrFakeClient.ApiV1beta1().FRRConfigurations(config.Kubernetes.OVNConfigNamespace).List(context.TODO(), metav1.ListOptions{})
				return len(list.Items)
			}, 2*time.Second).Should(gomega.Equal(1))

			// Verify base configuration
			baseConfig, err := frrFakeClient.ApiV1beta1().FRRConfigurations(config.Kubernetes.OVNConfigNamespace).Get(context.TODO(), BaseFRRConfigName, metav1.GetOptions{})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		ginkgo.It("should update base FRRConfiguration when a node is added", func() {
			...
			gomega.Eventually(func() int {
				bc, err := frrFakeClient.ApiV1beta1().FRRConfigurations(config.Kubernetes.OVNConfigNamespace).Get(context.TODO(), BaseFRRConfigName, metav1.GetOptions{})
				if err != nil {
					return 0
				}
		...
		ginkgo.It("should clean up stale FRRConfigurations", func() {
			...
			gomega.Eventually(func() bool {
				_, err := frrFakeClient.ApiV1beta1().FRRConfigurations(config.Kubernetes.OVNConfigNamespace).Get(context.TODO(), BaseFRRConfigName, metav1.GetOptions{})
				return err == nil
			...
			gomega.Eventually(func() int {
				list, _ := frrFakeClient.ApiV1beta1().FRRConfigurations(config.Kubernetes.OVNConfigNamespace).List(context.TODO(), metav1.ListOptions{
					LabelSelector: labels.FormatLabels(map[string]string{FRRConfigManagedLabel: "true"}),
				})
				return len(list.Items)
			}, 3*time.Second).Should(gomega.Equal(1))

			// Verify only base config remains
			configs, _ := frrFakeClient.ApiV1beta1().FRRConfigurations(config.Kubernetes.OVNConfigNamespace).List(context.TODO(), metav1.ListOptions{})
