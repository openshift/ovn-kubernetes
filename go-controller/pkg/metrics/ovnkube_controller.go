...
// or down status.
func MonitorIPsecTunnelsState(stopChan <-chan struct{}, wg *sync.WaitGroup, ovsDBClient libovsdbclient.Client, ipsec ipsecClient) error {
	if ovsDBClient == nil {
	...
	subscribe := func() (bool, chan []syscall.NetlinkMessage, *nl.NetlinkSocket, error) {
		...
		updateIPsecTunnelStateMetric(ovsDBClient, ipsec)
		return true, msgCh, sock, nil
	}

	return runInternalIPsec(stopChan, wg, ovsDBClient, ipsec, subscribe)
}
...
func runInternalIPsec(stopChan <-chan struct{}, wg *sync.WaitGroup,
	ovsDBClient libovsdbclient.Client, ipsec ipsecClient, subscribe xfrmSubscribeFn) error {
...
func updateIPsecTunnelStateMetric(ovsDBClient libovsdbclient.Client, ipsec ipsecClient) {
	...
	ipsecEstablished, err := areAllIPsecTunnelsEstablished(ipsec, geneveInterfaces)
	if err != nil {
