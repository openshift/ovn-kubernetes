...
func registerOvsMetrics(registry prometheus.Registerer) {
	registerOvsMetricsOnce.Do(func() {
		...
		go ovsDatapathMetricsUpdater(util.RunOVSAppctl, 30*time.Second, nil)
		// OVS bridge metrics updater
		go ovsBridgeMetricsUpdater(util.RunOVSVsctl, util.RunOVSOfctl, 30*time.Second, nil)
		// OVS interface metrics updater
		go ovsInterfaceMetricsUpdater(util.RunOVSVsctl, 30*time.Second, nil)
		// OVS memory metrics updater
		go ovsMemoryMetricsUpdater(util.RunOvsVswitchdAppCtl, 30*time.Second, nil)
		// OVS hw Offload metrics updater
		go ovsHwOffloadMetricsUpdater(util.RunOVSVsctl, 30*time.Second, nil)
