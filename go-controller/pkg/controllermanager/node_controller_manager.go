...
func (ncm *NodeControllerManager) Start(ctx context.Context, ovnkubeControllerStarted *atomic.Bool) (err error) {
	...
	// start temp fix: remove when ovn has native support for silencing GARPs / unsolicited route advertisements on startup
	// for LRPs
	for {
		select {
		case <-ctx.Done():
		...
		default:
			...
			ncm.defaultNodeNetworkController.Gateway.SetDefaultBridgeARPRplRouteAdvDropFlows(false)
			if err := ncm.defaultNodeNetworkController.Gateway.Reconcile(); err != nil {
				return fmt.Errorf("failed to reconcile gateway after removing arp reply / route advertisement drop flows for ext bridge: %v", err)
			}
			break
		}
	}
