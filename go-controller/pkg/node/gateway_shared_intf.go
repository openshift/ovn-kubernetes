...
func newGatewayOpenFlowManager(gwBridge, exGWBridge *bridgeConfiguration) (*openflowManager, error) {
	...
	if exGWBridge != nil {
		...
		exGWBridgeDftFlows := commonFlows(exGWBridge.ofPortPhys, exGWBridge.macAddress.String(),
			exGWBridge.ofPortPatch, exGWBridge.ofPortHost, exGWBridge.ips)
...
func commonFlows(ofPortPhys, bridgeMacAddress, ofPortPatch, ofPortHost string, bridgeIPs []*net.IPNet) []string {
	var dftFlows []string
	maxPktLength := getMaxFrameLength()
	...
	if config.IPv4Mode {
		...
		if config.Gateway.Mode == config.GatewayModeLocal {
			...
			// exceptions are traffic for egressIP and egressGW features which will hit the priority 100 flow instead of this.
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=495, in_port=%s, tcp, nw_src=%s, "+
					"actions=ct(table=4,zone=%d)",
					defaultOpenFlowCookie, ofPortPatch, physicalIP.IP, HostMasqCTZone))
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=495, in_port=%s, udp, nw_src=%s, "+
					"actions=ct(table=4,zone=%d)",
					defaultOpenFlowCookie, ofPortPatch, physicalIP.IP, HostMasqCTZone))
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=495, in_port=%s, sctp, nw_src=%s, "+
					"actions=ct(table=4,zone=%d)",
					defaultOpenFlowCookie, ofPortPatch, physicalIP.IP, HostMasqCTZone))
			// We send BFD traffic coming from OVN to outside directly
			dftFlows = append(dftFlows,
			...
			/*dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=650, table=0, in_port=%s, icmp, actions=output:%s",
					defaultOpenFlowCookie, ofPortPatch, ofPortPhys))*/
		}
	...
	if config.IPv6Mode {
		...
		if config.Gateway.Mode == config.GatewayModeLocal {
			...
			// exceptions are traffic for egressIP and egressGW features which will hit the priority 100 flow instead of this.
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=495, in_port=%s, tcp6, nw_src=%s, "+
					"actions=ct(table=4,zone=%d)",
					defaultOpenFlowCookie, ofPortPatch, physicalIP.IP, HostMasqCTZone))
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=495, in_port=%s, udp6, nw_src=%s, "+
					"actions=ct(table=4,zone=%d)",
					defaultOpenFlowCookie, ofPortPatch, physicalIP.IP, HostMasqCTZone))
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=495, in_port=%s, sctp6, nw_src=%s, "+
					"actions=ct(table=4,zone=%d)",
					defaultOpenFlowCookie, ofPortPatch, physicalIP.IP, HostMasqCTZone))
			// We send BFD traffic coming from OVN to outside directly
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=650, table=0, in_port=%s, udp6, tp_dst=3784, actions=output:%s",
					defaultOpenFlowCookie, ofPortPatch, ofPortPhys))
			/*dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=650, table=0, in_port=%s, icmp6, actions=output:%s",
					defaultOpenFlowCookie, ofPortPatch, ofPortPhys))*/
		}
		// table 0, packets coming from external. Send it through conntrack and
		// resubmit to table 1 to know the state and mark of the connection.
