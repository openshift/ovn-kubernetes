...
func flowsForDefaultBridge(bridge *bridgeconfig.BridgeConfiguration, extraIPs []net.IP) ([]string, error) {
	...
	if config.IPv6Mode {
		if ofPortPhys != "" {
			...
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=200, in_port=%s, udp6, udp_dst=%d, "+
					"actions=output:%s", defaultOpenFlowCookie, nodetypes.OvsLocalPort, config.Default.EncapPort, ofPortPhys))
		}
				...
				for _, subnet := range netConfig.NodeSubnets {
					...
					dftFlows = append(dftFlows,
						fmt.Sprintf("cookie=%s, priority=16, table=1, %s, %s_dst=%s, "+
							"actions=output:%s",
							defaultOpenFlowCookie, ipv, ipv, mgmtIP.IP, nodetypes.OvsLocalPort),
					)
				}
			}
		}
...
// primary interface.
func hostNetworkNormalActionFlows(netConfig *bridgeconfig.BridgeUDNConfiguration, srcMAC string, hostSubnets []*net.IPNet, isV6 bool) []string {
	var flows []string
	...
	for _, hostSubnet := range hostSubnets {
		...
		flows = append(flows, formatFlow(nodetypes.OvsLocalPort, hostSubnet.String(), ctMarkHost))
	}
	...
	if isV6 {
		...
		for _, icmpType := range []int{types.NeighborSolicitationICMPType, types.NeighborAdvertisementICMPType} {
			...
			flows = append(flows, formatICMPFlow(nodetypes.OvsLocalPort, ctMarkHost, icmpType))
		}
	}
	return flows
}
