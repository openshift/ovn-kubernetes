	...
	if util.IsNetworkSegmentationSupportEnabled() {
		...
		if util.IsUDNEnabledService(ktypes.NamespacedName{Namespace: service.Namespace, Name: service.Name}.String()) {
			...
			if util.IsRouteAdvertisementsEnabled() {
				...
				// sample flow: cookie=0xdeff105, duration=264.196s, table=0, n_packets=0, n_bytes=0, priority=500,ip,
				...
				flows = append(flows, fmt.Sprintf("cookie=%s, priority=500, in_port=%s, ip, ip_src=%s,actions=ct(zone=%d,nat,table=3)",
