...
func (npw *nodePortWatcher) createFlowsForNonLocalHostNetEp(cookie string, svcPort int32, flowProtocol string, externalIPOrLBIngressIP, nwDst, nwSrc string) []string {
	...
	if len(externalIPOrLBIngressIP) != 0 {
		...
		// if externalIPOrLBIngressIP is set then this service is of type NP
		nodeportFlowsResult = append(nodeportFlowsResult,
...
func addServiceRules(service *kapi.Service, svcHasLocalHostNetEndPnt bool, npw *nodePortWatcher) {
	...
	if npw != nil {
		npw.updateServiceFlowCache(service, addFlows, svcHasLocalHostNetEndPnt)
...
func delServiceRules(service *kapi.Service, npw *nodePortWatcher) {
	...
	// full mode || dpu mode
	if npw != nil {
		npw.updateServiceFlowCache(service, addFlows, false)
		npw.ofm.requestFlowSync()
		if !npw.dpuMode {
			...
			// case1: deletes the REDIRECT rules for etp=local + host-networked pods
			delGatewayIptRules(service, true)
			// case2: deletes the DNAT rules towards masqueradeIP for etp=local + ovn-k pods
			delGatewayIptRules(service, false)
			// case3: deletes the DNAT rules towards clusterIP
			delGatewayIptRules(service, false)
		}
		return
	}

	// For host only mode always try and delete rules here
	// case1: deletes the REDIRECT rules for etp=local + host-networked pods
	delGatewayIptRules(service, true)
	// case2: deletes the DNAT rules towards masqueradeIP for etp=local + ovn-k pods
	delGatewayIptRules(service, false)
