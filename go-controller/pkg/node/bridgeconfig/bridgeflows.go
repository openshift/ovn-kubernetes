...
func (b *BridgeConfiguration) flowsForDefaultBridge(extraIPs []net.IP) ([]string, error) {
	...
	if ofPortPhys != "" {
		for _, netConfig := range b.patchedNetConfigs() {
			...
			if config.IPv4Mode {
				// table 1, tracked & established and tracked & related connections in zone 64000 with ct_mark CtMarkOVN and the bridge MAC
				...
				// table 1, tracked & established and tracked & related connections in zone 64000 with ct_mark CtMarkOVN are applied a NORMAL action.
		...
		if config.IPv4Mode {
			// table 1, tracked & established and tracked & related connections in zone 64000 with ct_mark CtMarkHost and the bridge MAC
			...
			// table 1, tracked & established and tracked & related connections in zone 64000 with ct_mark CtMarkHost are applied a NORMAL action.
			// This allows reply traffic that is routed through an external router to be sent to its learned next hop rather
			// than directly to OVN.
			dftFlows = append(dftFlows,
