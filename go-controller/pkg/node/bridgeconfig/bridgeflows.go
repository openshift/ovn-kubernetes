...
func (b *BridgeConfiguration) commonFlows(hostSubnets []*net.IPNet) ([]string, error) {
	...
	if ofPortPhys != "" {
		...
		// table=0, n_packets=0, n_bytes=0, priority=104,ip,in_port=4,dl_src=02:42:ac:12:00:03,nw_src=23.100.0.0/16 actions=output:eth0
		// table=0, n_packets=0, n_bytes=0, priority=103,ip,in_port=4,nw_src=23.100.0.0/16 actions=drop
		for _, netConfig := range b.patchedNetConfigs() {
			...
			for _, clusterEntry := range netConfig.Subnets {
				...
				dftFlows = append(dftFlows, fmt.Sprintf("cookie=%s, priority=104, in_port=%s, dl_src=%s, %s, %s_src=%s, "+
					"actions=output:%s",
