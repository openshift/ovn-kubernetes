...
func (udng *UserDefinedNetworkGateway) delNetwork() error {
	...
	if config.IsModeFull() && udng.openflowManager != nil {
		...
		delUDNMasqIPNeighbors(bridgeName, udng.v4MasqIPs, udng.v6MasqIPs)
	}
...
func delUDNMasqIPNeighbors(bridgeName string, v4MasqIPs, v6MasqIPs *udn.MasqueradeIPs) {
	link, err := util.LinkSetUp(bridgeName)
	if err != nil {
		klog.Warningf("Unable to get link for %s to delete masquerade neighbors: %v", bridgeName, err)
		return
	}
	for _, ip := range udnMasqGatewayRouterIPs(v4MasqIPs, v6MasqIPs) {
		if err := util.LinkNeighDel(link, ip); err != nil {
			klog.Warningf("Failed to delete masquerade neighbor %s on %s: %v", ip, bridgeName, err)
		}
	}
}
