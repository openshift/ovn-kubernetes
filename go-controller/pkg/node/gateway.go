...
func gatewayInitInternal(nodeName, gwIntf, egressGatewayIntf string, gwNextHops []net.IP, nodeSubnets, gwIPs []*net.IPNet,
	...
	*bridgeConfiguration, *bridgeConfiguration, error) {
	...
	if egressGatewayIntf != "" {
		egressGWBridge, err = bridgeForInterface(egressGatewayIntf, nodeName, types.PhysicalNetworkExGwName, nodeSubnets, nil, false, gwNextHops)
		if err != nil {
...
func bridgeForInterface(intfName, nodeName,
	...
	advertised bool,
	gwNextHops []net.IP) (*bridgeConfiguration, error) {
	var intfRep string
	var err error
