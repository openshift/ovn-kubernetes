...
var _ = Describe("NetlinkDeviceManager", func() {
	...
	DescribeTable("fieldsMatch criticalMatch",
		func(existing netlink.Link, cfg *DeviceConfig, expectCriticalMismatch bool) {
			_, criticalMatch := fieldsMatch(existing, cfg.Link)
			Expect(!criticalMatch).To(Equal(expectCriticalMismatch))
		},
		// VRF
	...
	//
	// These tests enforce that fieldsMatch and buildManagedLink cover every
	// managed field. All other functions (linkMutableFieldsMatch, allFieldsEqual,
	// prepareLinkForModify, configsEqual) delegate to these two building blocks.
	// Adding a field to one without the other causes a test failure.
	...
	Describe("managed field exhaustiveness", func() {
		...
		It("each field is covered by fieldsMatch and buildManagedLink", func() {
			for _, f := range allFields {
				mutableMatch, criticalMatch := fieldsMatch(f.current, f.desired.Link)
				isMutable := !mutableMatch
				isCritical := !criticalMatch

				Expect(isMutable || isCritical).To(BeTrue(),
					"field %s is detected by neither mutable nor critical in fieldsMatch", f.name)

				// linkMutableFieldsMatch must agree with fieldsMatch
				Expect(linkMutableFieldsMatch(f.current, f.desired.Link)).To(Equal(mutableMatch),
					"linkMutableFieldsMatch disagrees with fieldsMatch for field %s", f.name)

				// prepareLinkForModify delegates to buildManagedLink
				...
				// buildManagedLink must carry every field that fieldsMatch checks
				managed := buildManagedLink(f.desired.Link)
				managedMutable, managedCritical := fieldsMatch(managed, f.desired.Link)
				Expect(managedMutable).To(BeTrue(),
					"buildManagedLink does not carry mutable field %s", f.name)
				Expect(managedCritical).To(BeTrue(),
					"buildManagedLink does not carry critical field %s", f.name)
			}
		})
	})
	...
	Describe("upstream field audit", func() {
		...
		It("managed maps match buildManagedLink", func() {
				...
				for i := 0; i < rv.NumField(); i++ {
					...
					if managed[f.Name] {
						Expect(rv.Field(i).IsZero()).To(BeFalse(),
							"managed map lists %s.%s but buildManagedLink zeroes it", typ.Name(), f.Name)
					} else {
						Expect(rv.Field(i).IsZero()).To(BeTrue(),
							"managed map does not list %s.%s but buildManagedLink preserves it", typ.Name(), f.Name)
					}
				}
			}
		})
