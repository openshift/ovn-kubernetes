...
func getGatewayIPTRules(service *kapi.Service, nodePortTargetIP string, nodeIP *net.IPNet) []iptRule {
	...
	for _, svcPort := range service.Spec.Ports {
		if util.ServiceTypeHasNodePort(service) {
			...
			// OCP TEMP FIX: fixed upstream by https://github.com/ovn-org/ovn-kubernetes/pull/1512
			rules = append(rules, getNodePortIPTRules(svcPort, nodeIP, nodePortTargetIP, svcPort.NodePort)...)
			// END OCP TEMP FIX
			// OCP HACK: Fix Azure/GCP LoadBalancers. https://github.com/openshift/ovn-kubernetes/pull/112
			rules = append(rules, getLoadBalancerIPTRules(service, svcPort, nodePortTargetIP, svcPort.NodePort)...)
			// END OCP HACK
		}
