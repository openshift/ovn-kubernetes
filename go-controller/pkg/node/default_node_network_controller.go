...
func (nc *DefaultNodeNetworkController) Stop() {
	...
	if nc.Gateway != nil {
		nc.Gateway.SetDefaultBridgeARPRplRouteAdvDropFlows(true)
		if err := nc.Gateway.Reconcile(); err != nil {
			klog.Errorf("Failed to reconcile gateway after attempting to add flows to the external bridge to drop ARP request / route advertisement: %v", err)
		}
	}
	close(nc.stopChan)
	nc.wg.Wait()
