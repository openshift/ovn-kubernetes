		...
		Schema: spec.Schema{
			SchemaProps: spec.SchemaProps{
				...
				Properties: map[string]spec.Schema{
					...
					"egressNodeSelector": {
						SchemaProps: spec.SchemaProps{
							Description: "EgressNodeSelector limits the pool of nodes that can host the EgressIPs in this object; it applies to all EgressIPs in this object. Only nodes whose labels match this selector are eligible for assignment. This field is optional. When not specified by the user, the CRD default is applied: {matchExpressions: [{key: k8s.ovn.org/egress-assignable, operator: Exists}]}. An empty selector ({}) matches all nodes in the cluster including control plane nodes — use with caution as it expands the eligible node pool and health-check set to the entire cluster. This field restricts only where the IPs in this EgressIP object are placed; it does not reserve the matched nodes for exclusive use. Other EgressIP objects can still consume capacity on the same nodes.",
							Ref:         ref(metav1.LabelSelector{}.OpenAPIModelName()),
						},
					},
				},
