...
func (ovn *Controller) getGRLogicalSwitchForLoadBalancer(lb string) (string, error) {
	...
	if strings.HasPrefix(out, util.ExternalSwitchPrefix) {
		routerName := strings.TrimPrefix(out, util.ExternalSwitchPrefix)
		return util.ExternalSwitchPrefix + routerName, nil
	}
