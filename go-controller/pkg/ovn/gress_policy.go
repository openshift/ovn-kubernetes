...
func (gp *gressPolicy) localPodSetACL(portGroupName, portGroupUUID string) {
	...
	if len(gp.portPolicies) == 0 {
		...
		if len(gp.ipBlock) > 0 {
			...
			for i, cidrMatch := range cidrMatches {
				if err := gp.addOrModifyACLAllow(cidrMatch, l4Match, portGroupUUID, i+1); err != nil {
		...
		if gp.sizeOfAddressSet() > 0 || len(gp.ipBlock) == 0 {
			if err := gp.addOrModifyACLAllow(match, l4Match, portGroupUUID, 0); err != nil {
				klog.Warningf(err.Error())
			}
		}
	}
