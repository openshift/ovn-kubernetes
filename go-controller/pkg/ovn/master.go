...
func (oc *Controller) allocateNodeSubnets(node *kapi.Node) ([]*net.IPNet, []*net.IPNet, error) {
	...
	for _, subnet := range hostSubnets {
		...
		_ = oc.masterSubnetAllocator.ReleaseNetwork(subnet)
	}
		...
		if allocatedHostSubnet != nil {
			...
			defer func() {
				if err != nil {
					...
					_ = oc.masterSubnetAllocator.ReleaseNetwork(allocatedHostSubnet)
				}
			}()
		}
	}
	...
	defer func() {
		if err != nil {
			for _, allocatedSubnet := range allocatedSubnets {
				...
				_ = oc.masterSubnetAllocator.ReleaseNetwork(allocatedSubnet)
			}
		}
	}()
