...
func (c *lbConfig) makeNodeSwitchTargetIPs(service *v1.Service, node *nodeInfo, endpoints []discovery.Endpoint) (targetIPs []string, changed bool) {
	allTargetIPs := util.GetEligibleEndpointAddresses(endpoints, service)
	...
	if c.externalTrafficLocal || c.internalTrafficLocal {
		...
		targetIPs = util.GetLocalEligibleEndpointAddresses(endpoints, service, node.name)
	}
...
func buildServiceLBConfigs(service *v1.Service, endpointSlices []*discovery.EndpointSlice, useLBGroup, useTemplates bool) (perNodeConfigs, templateConfigs, clusterConfigs []lbConfig) {
	...
	for _, svcPort := range service.Spec.Ports {
		eps := util.GetCandidateLbEndpoints(endpointSlices, svcPort, service)
...
func buildClusterLBs(service *v1.Service, configs []lbConfig, nodeInfos []nodeInfo, useLBGroup bool) []LB {
	...
	for _, proto := range protos {
		...
		for _, config := range cfgs {
			...
			v4targets := []Addr{}
			for _, targetIP := range util.GetEligibleEndpointAddresses(config.eps.V4Endpoints, service) {
			...
			v6targets := []Addr{}
			for _, targetIP := range util.GetEligibleEndpointAddresses(config.eps.V6Endpoints, service) {
...
func buildTemplateLBs(service *v1.Service, configs []lbConfig, nodes []nodeInfo,
	...
	for _, proto := range protos {
		...
		for _, config := range configs {
			...
			allV4TargetIPs := util.GetEligibleEndpointAddresses(config.eps.V4Endpoints, service)
			allV6TargetIPs := util.GetEligibleEndpointAddresses(config.eps.V6Endpoints, service)
		...
		for _, proto := range protos {
			...
			for _, config := range configs {
				...
				if service.Namespace == "openshift-dns" && service.Name == "dns-default" {
					...
					switchV4targetDNSips := util.GetLocalEligibleEndpointAddresses(config.eps.V4Endpoints, service, node.name)
					// switchV6targetDNSips := util.FilterIPsSlice(allV6TargetIPs, node.podSubnets, true)
					switchV6targetDNSips := util.GetLocalEligibleEndpointAddresses(config.eps.V6Endpoints, service, node.name)
