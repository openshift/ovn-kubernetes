...
func (c *lbConfig) makeNodeSwitchTargetIPs(service *v1.Service, node *nodeInfo, epIPs []string) []string {
	targetIPs := epIPs
...
func (c *lbConfig) makeNodeRouterTargetIPs(service *v1.Service, node *nodeInfo, epIPs []string, hostMasqueradeIP string) ([]string, bool) {
	targetIPs := epIPs
	...
	return targetIPs, zeroRouterLocalEndpoints
}
		...
		for _, config := range configs {
			...
			for range config.vips {
				...
				for _, node := range nodes {
					switchV4targetips := config.makeNodeSwitchTargetIPs(service, &node, config.eps.V4IPs)
					switchV6targetips := config.makeNodeSwitchTargetIPs(service, &node, config.eps.V6IPs)

					routerV4targetips, _ := config.makeNodeRouterTargetIPs(service, &node, config.eps.V4IPs, types.V4HostMasqueradeIP)
					routerV6targetips, _ := config.makeNodeRouterTargetIPs(service, &node, config.eps.V6IPs, types.V6HostMasqueradeIP)
	...
	for _, node := range nodes {
		for _, proto := range protos {
			...
			for _, config := range configs {
				...
				routerV4targetips, zeroRouterV4LocalEndpoints := config.makeNodeRouterTargetIPs(service, &node, config.eps.V4IPs, types.V4HostMasqueradeIP)
				routerV6targetips, zeroRouterV6LocalEndpoints := config.makeNodeRouterTargetIPs(service, &node, config.eps.V6IPs, types.V6HostMasqueradeIP)
