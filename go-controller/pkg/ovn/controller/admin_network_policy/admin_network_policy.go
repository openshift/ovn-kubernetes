...
func (c *Controller) ensureAdminNetworkPolicy(anp *anpapi.AdminNetworkPolicy) error {
	...
	desiredACLs := c.getACLsOfRules(desiredANPState, portGroupName)
	...
	if len(currentANPState.egressRules) != len(desiredANPState.egressRules) ||
		...
		ops, err = c.constructOpsForRuleChanges(currentANPState, desiredANPState)
		if err != nil {
	...
	if !fullPeerRecompute && !reflect.DeepEqual(desiredANPState.ingressRules, currentANPState.ingressRules) {
		addrOps, err := c.constructOpsForPeerChanges(desiredANPState.ingressRules, currentANPState.ingressRules, &atLeastOneRuleUpdated, desiredANPState.name)
		if err != nil {
	...
	if !fullPeerRecompute && !reflect.DeepEqual(desiredANPState.egressRules, currentANPState.egressRules) {
		addrOps, err := c.constructOpsForPeerChanges(desiredANPState.egressRules, currentANPState.egressRules, &atLeastOneRuleUpdated, desiredANPState.name)
		if err != nil {
...
func (c *Controller) getACLsOfRules(anp *adminNetworkPolicyState, pgName string) []*nbdb.ACL {
	acls := []*nbdb.ACL{}
	for _, ingressRule := range anp.ingressRules {
		acl := c.convertANPRuleToACL(ingressRule, pgName, anp.name)
		acls = append(acls, acl...)
	}
	for _, egressRule := range anp.egressRules {
		acl := c.convertANPRuleToACL(egressRule, pgName, anp.name)
		acls = append(acls, acl...)
	}
...
func (c *Controller) convertANPRuleToACL(rule *gressRule, pgName, anpName string) []*nbdb.ACL {
	...
	if len(rule.ports) == 0 {
		...
		acl := libovsdbutil.BuildANPACL(
			getANPRuleACLDbIDs(anpName, rule.gressPrefix, fmt.Sprintf("%d", rule.gressIndex), c.controllerName),
			int(rule.priority),
			match,
			rule.action,
	...
	for i, port := range rule.ports {
		...
		acl := libovsdbutil.BuildANPACL(
			getANPRuleACLDbIDs(anpName, rule.gressPrefix, fmt.Sprintf("%d.%d", rule.gressIndex, i), c.controllerName),
			int(rule.priority),
			match,
			rule.action,
			libovsdbutil.ACLDirectionToACLPipeline(libovsdbutil.ACLDirection(rule.gressPrefix)),
...
func (c *Controller) createNewANP(desiredANPState *adminNetworkPolicyState, desiredACLs []*nbdb.ACL, desiredPorts []*nbdb.LogicalSwitchPort) error {
	...
	addrSetOps, err := c.constructOpsForNewPeer(desiredANPState)
	if err != nil {
...
func (c *Controller) constructOpsForRuleChanges(currentANPState, desiredANPState *adminNetworkPolicyState) ([]ovsdb.Operation, error) {
	var ops []ovsdb.Operation
	var err error
...
func (c *Controller) constructOpsForNewPeer(desiredANPState *adminNetworkPolicyState) ([]ovsdb.Operation, error) {
	var ops []ovsdb.Operation
...
func (c *Controller) constructOpsForPeerChanges(desiredRules, currentRules []*gressRule, atLeastOneRuleUpdated *bool, anpName string) ([]ovsdb.Operation, error) {
	var ops []ovsdb.Operation
