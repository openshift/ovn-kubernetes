...
var _ = ginkgo.Describe("Egress Firewall External DNS Operations", func() {
	...
	ginkgo.BeforeEach(func() {
		...
		fakeClient = util.GetOVNClientset().GetMasterClientset()
		var err error
		wf, err = factory.NewMasterWatchFactory(fakeClient)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())

		extEgDNS, err = NewExternalEgressDNS(fakeAddressSetFactory, DefaultNetworkControllerName, true, wf.DNSNameResolverInformer().Informer())
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
