...
func (gw *GatewayManager) GatewayInit(
	...
	// DisableSNATMultipleGWs is only applicable to cluster default network and not to user defined networks. In this case
	// we only add SNAT rules if the network is advertised.
	// For user defined networks, we always add SNAT rules regardless of whether the network is advertised or not.
	if (!config.Gateway.DisableSNATMultipleGWs && !gw.isRoutingAdvertised(nodeName)) || gw.netInfo.IsPrimaryNetwork() {
...
func (gw *GatewayManager) GetNetworkScopedClusterSubnetSNATMatch(nodeName string, isNetworkAdvertised bool) (string, error) {
	if gw.netInfo.IsDefault() {
		// for default network, we don't need to add SNAT match conditionally.
		return "", nil
	}
