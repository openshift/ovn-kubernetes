	...
	h, err := oc.watchFactory.AddFilteredPodHandler(policy.Namespace,
		...
		cache.ResourceEventHandlerFuncs{
			AddFunc: func(obj interface{}) {
				pod := obj.(*kapi.Pod)
				logrus.Infof("#### [%s/%s] PP add pod matches NP %s/%s", pod.Namespace, pod.Name, policy.Namespace, policy.Name)
				oc.handlePeerPodSelectorAddUpdate(np, addressMap, addressSet, obj)
			},
			DeleteFunc: func(obj interface{}) {
				pod := obj.(*kapi.Pod)
				logrus.Infof("#### [%s/%s] PP delete pod matches NP %s/%s", pod.Namespace, pod.Name, policy.Namespace, policy.Name)
				oc.handlePeerPodSelectorDelete(np, addressMap, addressSet, obj)
			},
			UpdateFunc: func(oldObj, newObj interface{}) {
				pod := newObj.(*kapi.Pod)
				logrus.Infof("#### [%s/%s] PP update pod matches NP %s/%s", pod.Namespace, pod.Name, policy.Namespace, policy.Name)
	...
	namespaceHandler, err := oc.watchFactory.AddFilteredNamespaceHandler("",
		...
		cache.ResourceEventHandlerFuncs{
			AddFunc: func(obj interface{}) {
				...
				logrus.Infof("%s NAP add matching namespace %s", npdesc(policy), namespace.Name)
				np.Lock()
				...
				podHandler, err := oc.watchFactory.AddFilteredPodHandler(namespace.Name,
					...
					cache.ResourceEventHandlerFuncs{
						AddFunc: func(obj interface{}) {
							pod := obj.(*kapi.Pod)
							logrus.Infof("#### [%s/%s] add pod matches NP %s/%s", pod.Namespace, pod.Name, policy.Namespace, policy.Name)
							oc.handlePeerPodSelectorAddUpdate(np, addressMap, addressSet, obj)
						},
						DeleteFunc: func(obj interface{}) {
							pod := obj.(*kapi.Pod)
							logrus.Infof("#### [%s/%s] delete pod matches NP %s/%s", pod.Namespace, pod.Name, policy.Namespace, policy.Name)
							oc.handlePeerPodSelectorDelete(np, addressMap, addressSet, obj)
							oc.handlePeerPodSelectorDeleteACLRules(obj, gress)
						},
						UpdateFunc: func(oldObj, newObj interface{}) {
							pod := newObj.(*kapi.Pod)
							logrus.Infof("#### [%s/%s] update pod matches NP %s/%s", pod.Namespace, pod.Name, policy.Namespace, policy.Name)
							oc.handlePeerPodSelectorAddUpdate(np, addressMap, addressSet, newObj)
						},
					}, nil)
				if err != nil {
			...
			DeleteFunc: func(obj interface{}) {
				namespace := obj.(*kapi.Namespace)
				logrus.Infof("%s NAP delete matching namespace %s", npdesc(policy), namespace.Name)
			},
			UpdateFunc: func(oldObj, newObj interface{}) {
				namespace := newObj.(*kapi.Namespace)
				logrus.Infof("%s NAP update matching namespace %s", npdesc(policy), namespace.Name)
			},
		}, nil)
	if err != nil {
...
func (oc *Controller) handlePeerNamespaceSelector(
	...
	h, err := oc.watchFactory.AddFilteredNamespaceHandler("",
		...
		cache.ResourceEventHandlerFuncs{
			AddFunc: func(obj interface{}) {
				namespace := obj.(*kapi.Namespace)
				logrus.Infof("%s NP add matching namespace %s", npdesc(policy), namespace.Name)
				np.Lock()
				defer np.Unlock()
				if np.deleted {
					return
			...
			DeleteFunc: func(obj interface{}) {
				namespace := obj.(*kapi.Namespace)
				logrus.Infof("%s NP delete matching namespace %s", npdesc(policy), namespace.Name)
				np.Lock()
				defer np.Unlock()
				if np.deleted {
					return
			...
			UpdateFunc: func(oldObj, newObj interface{}) {
				namespace := newObj.(*kapi.Namespace)
				logrus.Infof("%s NP update matching namespace %s", npdesc(policy), namespace.Name)
			},
		}, nil)
	if err != nil {
		logrus.Errorf("error watching namespaces for policy %s: %v",
