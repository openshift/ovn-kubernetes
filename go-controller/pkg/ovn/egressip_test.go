	...
	getIPv4Nodes := func(nodeInfos []nodeInfo) []v1.Node {
		...
		for i, ni := range nodeInfos {
			...
			annotations := map[string]string{
				...
				"k8s.ovn.org/host-routable-subnets":           fmt.Sprintf("%s", hostAddresses),
			...
			func(interconnect, isnode1Local, isnode2Local bool) {
				...
				app.Action = func(ctx *cli.Context) error {
					...
					annotations = map[string]string{
						...
						"k8s.ovn.org/host-routable-subnets":           fmt.Sprintf("[\"%s\",\"%s\"]", node2IPv4, node2IPv6),
					}
			...
			app.Action = func(ctx *cli.Context) error {
				...
				node1 := v1.Node{
					ObjectMeta: metav1.ObjectMeta{
						Name: node1Name,
						Annotations: map[string]string{
							"k8s.ovn.org/host-routable-subnets": fmt.Sprintf("[\"%s\", \"%s\", \"%s\", \"%s\"]", node1IPv4CIDR, node1IPv6CIDR, vipIPv4CIDR, vipIPv6CIDR),
						},
					},
				...
				node2 := v1.Node{
					ObjectMeta: metav1.ObjectMeta{
						Name: node2Name,
						Annotations: map[string]string{
							"k8s.ovn.org/host-routable-subnets": fmt.Sprintf("[\"%s\", \"%s\"]", node2IPv4CIDR, node2IPv6CIDR),
						},
					},
					Status: v1.NodeStatus{
						Conditions: []v1.NodeCondition{
				...
				node1.ObjectMeta.Annotations["k8s.ovn.org/host-routable-subnets"] = fmt.Sprintf("[\"%s\", \"%s\"]", node1IPv4CIDR, node1IPv6CIDR)
				_, err = fakeOvn.fakeClient.KubeClient.CoreV1().Nodes().Update(context.TODO(), &node1, metav1.UpdateOptions{})
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				...
				node2.ObjectMeta.Annotations["k8s.ovn.org/host-routable-subnets"] = fmt.Sprintf("[\"%s\", \"%s\", \"%s\", \"%s\"]", node2IPv4CIDR, node2IPv6CIDR, vipIPv4CIDR, vipIPv6CIDR)
				_, err = fakeOvn.fakeClient.KubeClient.CoreV1().Nodes().Update(context.TODO(), &node2, metav1.UpdateOptions{})
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			...
			func(interconnect bool, node1Zone, node2Zone string) {
				...
				app.Action = func(ctx *cli.Context) error {
					...
					annotations := map[string]string{
						...
						"k8s.ovn.org/host-routable-subnets":           fmt.Sprintf("[\"%s\"]", node1IPv4CIDR),
					}
					if node1Zone != "global" {
						annotations["k8s.ovn.org/remote-zone-migrated"] = node1Zone // used only for ic=true test
					}
					...
					annotations = map[string]string{
						...
						"k8s.ovn.org/node-transit-switch-port-ifaddr": "{\"ipv4\":\"168.254.0.3/16\"}", // used only for ic=true test
						"k8s.ovn.org/zone-name":                       node2Zone,                       // used only for ic=true test
						"k8s.ovn.org/host-routable-subnets":           fmt.Sprintf("[\"%s\"]", node2IPv4CIDR),
					}
					if node2Zone != "global" {
						annotations["k8s.ovn.org/remote-zone-migrated"] = node2Zone // used only for ic=true test
					}
		...
		ginkgo.It("should populated egress node data as they are tagged `egress assignable` with variants of IPv4/IPv6", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				annotations := map[string]string{
					...
					"k8s.ovn.org/node-subnets":          fmt.Sprintf("{\"default\":[\"%s\", \"%s\"]}", v4Node1Subnet, v6Node1Subnet),
					"k8s.ovn.org/host-routable-subnets": fmt.Sprintf("[\"%s\",\"%s\"]", node1IPv4CIDR, node1IPv6CIDR),
				}
				...
				node := v1.Node{
					ObjectMeta: metav1.ObjectMeta{
						...
						Annotations: map[string]string{
							...
							"k8s.ovn.org/node-subnets":          fmt.Sprintf("{\"default\":[\"%s\", \"%s\"]}", v4Node1Subnet, v6Node1Subnet),
							"k8s.ovn.org/host-routable-subnets": fmt.Sprintf("[\"%s\",\"%s\"]", nodeIPv4CIDR, nodeIPv6CIDR),
						},
					},
					Status: v1.NodeStatus{
						Conditions: []v1.NodeCondition{
