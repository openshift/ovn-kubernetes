...
var _ = ginkgo.Describe("OVN master EgressIP Operations", func() {
	...
	ginkgo.Context("WatchEgressNodes running with WatchEgressIP", func() {
		...
		ginkgo.It("should re-balance EgressIPs when their node is removed", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				// TODO: had to place pod on node3 instead of node1, otherwise when node1 is deleted, nexthop in LRP (line no 11561) for the pod
				// doesn't get updated with newly added node node2 (100.64.0.3).
				// This https://github.com/openshift/ovn-kubernetes/blob/release-4.16/go-controller/pkg/ovn/egressip.go#L1522 doesn't hit.
				egressPod1 := *newPodWithLabels(eipNamespace, podName, node3Name, podV4IP, egressPodLabel)
				egressNamespace := newNamespace(eipNamespace)
				annotations := map[string]string{
					"k8s.ovn.org/node-primary-ifaddr": fmt.Sprintf("{\"ipv4\": \"%s\", \"ipv6\": \"%s\"}", node1IPv4CIDR, node1IPv6CIDR),
					"k8s.ovn.org/node-subnets":        fmt.Sprintf("{\"default\":[\"%s\", \"%s\"]}", v4Node1Subnet, v6Node1Subnet),
				...
				nodeIPsASdbIDs := getEgressIPAddrSetDbIDs(NodeIPAddrSetName, DefaultNetworkControllerName)
				fakeOvn.asf.EventuallyExpectAddressSetWithAddresses(nodeIPsASdbIDs, []string{node1IPv4, node1IPv6, node3IPv4})
	...
	ginkgotable.DescribeTable("remove invalid next hop from LRP", func(isV6 bool) {
		...
		app.Action = func(*cli.Context) error {
			...
			fakeOvn.startWithDBSetup(
				libovsdbtest.TestSetup{
					NBData: []libovsdbtest.TestData{
						...
						getReRoutePolicy(podIP, getSupportedOVNIPFamily(), "stale-reroute-UUID", staleHops, eipExternalID),
			...
			expectedDatabaseState := []libovsdbtest.TestData{
				...
				getReRoutePolicy(podIP, getSupportedOVNIPFamily(), "valid-reroute-UUID", validNextHops, eipExternalID),
				&nbdb.LogicalRouter{
					Name:     types.OVNClusterRouter,
					UUID:     types.OVNClusterRouter + "-UUID",
		...
		err := app.Run([]string{app.Name})
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	}, ginkgotable.Entry("IPv4", false), ginkgotable.Entry("IPv6", true))
	...
	ginkgotable.DescribeTable(
		...
		func(includeUnscheduled, includeScheduledButNoIP, includeHostNetwork, interconnect, isPodRemote bool) {
			app.Action = func(*cli.Context) error {
				...
				nodeIPsASdbIDs := getEgressIPAddrSetDbIDs(NodeIPAddrSetName, DefaultNetworkControllerName)
				eipServedPodsDbIDs := getEgressIPAddrSetDbIDs(EgressIPServedPodsAddrSetName, DefaultNetworkControllerName)
				egressServiceDbIDs := egresssvc.GetEgressServiceAddrSetDbIDs(DefaultNetworkControllerName)
				...
				gomega.Eventually(fakeOvn.nbClient).Should(libovsdbtest.HaveData(expectedDatabaseState))
				fakeOvn.asf.EventuallyExpectAddressSetWithAddresses(nodeIPsASdbIDs, []string{node1IPv4, node2IPv4})
				fakeOvn.asf.EventuallyExpectAddressSetWithAddresses(eipServedPodsDbIDs, []string{podV4IP, podV4IP2})
				fakeOvn.asf.EventuallyExpectAddressSetWithAddresses(egressServiceDbIDs, nil)
				...
				for i := 0; i < 10; i++ {
					...
					expectedDatabaseState = []libovsdbtest.TestData{
						...
						&nbdb.LogicalRouter{
							Name:     types.OVNClusterRouter,
							UUID:     types.OVNClusterRouter + "-UUID",
							Policies: []string{"no-reroute-UUID", "no-reroute-service-UUID", "default-no-reroute-node-UUID", "reroute-UUID1", "reroute-UUID2", "egressip-no-reroute-reply-traffic"},
						},
						&nbdb.LogicalRouter{
							Name:    types.GWRouterPrefix + node1.Name,
							UUID:    types.GWRouterPrefix + node1.Name + "-UUID",
		...
		ginkgotable.Entry("unscheduled pending pod only, interconnect disabled", true, false, false, false, false),
		ginkgotable.Entry("scheduled pending pod only, interconnect disabled", false, true, false, false, false),
		ginkgotable.Entry("host-networked pod only, interconnect disabled", false, false, true, false, false),
		ginkgotable.Entry("unscheduled pending pod only, interconnect enabled, local pod", true, false, false, true, false),
		ginkgotable.Entry("scheduled pending pod only, interconnect enabled, local pod", false, true, false, true, false),
		ginkgotable.Entry("host-networked pod only, interconnect enabled, local pod", false, false, true, true, false),
		ginkgotable.Entry("unscheduled pending pod only, interconnect enabled, remote pod", true, false, false, true, true),
		ginkgotable.Entry("scheduled pending pod only, interconnect enabled, remote pod", false, true, false, true, true),
		ginkgotable.Entry("host-networked pod only, interconnect enabled, remote pod", false, false, true, true, true),
	)
	...
	ginkgo.It("should update SNAT and LRP nexthops during simultaneous EIP failover and ovnkube-controller restart", func() {
		app.Action = func(*cli.Context) error {
			...
			nodeIPsASdbIDs := getEgressIPAddrSetDbIDs(NodeIPAddrSetName, DefaultNetworkControllerName)
			fakeOvn.asf.EventuallyExpectAddressSetWithAddresses(nodeIPsASdbIDs, []string{node1IPv4, node2IPv4, node3IPv4})
			...
			err = fakeOvn.controller.reconcileEgressIP(nil, &eIP)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			...
			expectedDatabaseState := []libovsdbtest.TestData{
				...
				&nbdb.LogicalRouter{
					Name:     types.OVNClusterRouter,
					UUID:     types.OVNClusterRouter + "-UUID",
					Policies: []string{"no-reroute-UUID", "no-reroute-service-UUID", "default-no-reroute-node-UUID", "egressip-no-reroute-reply-traffic", "reroute-UUID1"},
				},
				&nbdb.LogicalRouter{
					Name:    types.GWRouterPrefix + node1.Name,
					UUID:    types.GWRouterPrefix + node1.Name + "-UUID",
