...
func (oc *DefaultNetworkController) interconnectAddUpdateLocalNode(node *kapi.Node) error {
...
func (oc *DefaultNetworkController) interconnectCreateLocalNodeResources(node *kapi.Node, nodeId int) error {
	...
	if err := libovsdbops.CreateOrUpdateLogicalRouterPort(oc.nbClient, &logicalRouter,
...
func (oc *DefaultNetworkController) addNodeLogicalSwitchPort(logicalSwitchName, portName, portType string, addresses []string, options map[string]string) error {
	logicalSwitch := nbdb.LogicalSwitch{
		Name: types.TransitSwitch,
	}
...
func (oc *DefaultNetworkController) interconnectAddUpdateRemoteNode(node *kapi.Node) error {
	nodeId := util.GetNodeId(node)
	if nodeId == -1 {
		// Don't consider this node as cluster-manager has not allocated node id yet.
		return fmt.Errorf("failed to get node id for node - %s", node.Name)
...
func (oc *DefaultNetworkController) interconnectCreateRemoteNodeResources(node *kapi.Node, nodeId int, chassisId string) error {
	nodeTransitSwitchPortIps, err := util.ParseNodeTransitSwitchPortAddresses(node)
	if err != nil || len(nodeTransitSwitchPortIps) == 0 {
		return fmt.Errorf("failed to get the node transit switch port Ips : %v", err)
	}
	...
	logicalRouterPort, err := libovsdbops.GetLogicalRouterPort(oc.nbClient, &lrp)
	if err != nil {
		return nil
	}
...
func (oc *DefaultNetworkController) setRemotePortBindingChassis(nodeName, portName, chassisId string) error {
	...
	if err := libovsdbops.UpdatePortBindingSetChassis(oc.sbClient, &remotePort, &chassis); err != nil {
		return fmt.Errorf("failed to update chassis for remote port, error: %v", err)
	}

	return nil
}

func (ic *DefaultNetworkController) addRemoteNodeStaticRoutes(node *kapi.Node, nodeId int, tsIps []string) error {
	addRoute := func(subnet *net.IPNet, tsIp string) error {
		...
		if err := libovsdbops.CreateOrReplaceLogicalRouterStaticRouteWithPredicate(ic.nbClient, types.OVNClusterRouter, &logicalRouterStaticRoute, p); err != nil {
	...
	joinSubnets, err := util.ParseZoneJoinSubnetsAnnotation(node, types.DefaultNetworkName)
	if err != nil {
...
func (ic *DefaultNetworkController) deleteLocalNodeStaticRoutes(node *kapi.Node, nodeId int, tsIps []string) error {
...
func (oc *DefaultNetworkController) interconnectRemoveNode(node *kapi.Node) error {
	//TODO
	return nil
}
