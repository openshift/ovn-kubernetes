...
var _ = Describe("BaseUserDefinedNetworkController", func() {
	...
	It("should not delete localnet LSPs for pods whose NADs are not yet registered", func() {
		...
		// via HasNAD(). Pods using later-arriving NADs are skipped, and their existing
		// LSPs get deleted as stale.
		...
		// Create 4 ipamless localnet NADs all for the same network (customer scenario).
		// Each NAD needs the network-name annotation that the cluster manager sets.
		...
		// LSP names: prefix is derived from the NAD name (namespace/name)
		...
		// Pre-populate OVN DB with all 4 LSPs on the localnet switch (simulating pre-restart state)
		...
		fakeOVN.startWithDBSetup(
			...
			// Add only nad0 via the standard mechanism. nad1-3 will be added to
			// the NAD informer cache after controller creation to simulate the race:
			// NADs exist in API server but aren't registered with the controller yet.
		...
		for _, nad := range []*nadapi.NetworkAttachmentDefinition{nad1, nad2, nad3} {
			...
			if err != nil {
				// NAD not in cache yet, add it via the NAD client tracker
		...
		// This simulates the race: controller starts before vm1, vm2, vm3 NADs are registered.
		...
		Expect(len(localnetSW.Ports)).To(Equal(4),
