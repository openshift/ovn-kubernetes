...
func (bnc *BaseNetworkController) ensureNetworkInfoForVM(pod *corev1.Pod) error {
	if !kubevirt.PodIsLiveMigratable(pod) {
		return nil
	}
	vmNetworkInfo, err := kubevirt.FindNetworkInfo(bnc.watchFactory, pod)
	if err != nil {
		return err
	}
	resultErr := retry.RetryOnConflict(util.OvnConflictBackoff, func() error {
		// Informer cache should not be mutated, so get a copy of the object
		pod, err := bnc.watchFactory.GetPod(pod.Namespace, pod.Name)
		if err != nil {
			return err
		}
