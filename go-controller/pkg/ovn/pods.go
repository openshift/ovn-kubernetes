...
func (oc *Controller) addLogicalPort(pod *kapi.Pod) error {
	...
	if err == nil {
		...
		start4 := time.Now()
		logrus.Infof("[%s/%s] addLogicalPort after port-sec %v [+%v since wait addrs]", pod.Namespace, pod.Name, time.Since(start), time.Since(start3))
		oc.logicalPortCache[portName] = logicalSwitch
		oc.addPodToNamespace(pod.Namespace, annotation.IP.IP, portName)
		logrus.Infof("[%s/%s] addLogicalPort after addToNamespace %v [+%v since port-sec]", pod.Namespace, pod.Name, time.Since(start), time.Since(start4))
		return nil
	}
