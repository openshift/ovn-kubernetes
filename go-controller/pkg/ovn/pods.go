...
// Builds the logical switch port name for a given pod.
func podLogicalPortNameSplit(namespace, name string) string {
...
func (oc *Controller) deleteLogicalPort(pe *podCacheEntry) error {
	...
	logicalPort := podLogicalPortNameSplit(pe.Namespace, pe.Name)
	...
	if err := oc.deletePodFromNamespace(pe.Namespace, portInfo.name, portInfo.uuid, portInfo.ips); err != nil {
		klog.Errorf(err.Error())
	}
