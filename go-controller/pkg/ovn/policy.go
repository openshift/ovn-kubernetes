...
func (oc *Controller) syncNetworkPolicies(networkPolicies []interface{}) {
	...
	uuids, stderr, err := util.RunOVNNbctl("--data=bare", "--no-heading",
		"--columns=_uuid", "find", "ACL", "direction="+toLport,
	...
	for _, uuid := range updateList {
		...
		if _, stderr, err := util.RunOVNNbctl("set", "acl", uuid,
			fmt.Sprintf("direction=%s", fromLport), "options:apply-after-lb=true"); err != nil {
...
func addACLPortGroup(portGroupUUID, direction, priority, match, action string, policyType knet.PolicyType) error {
	...
	createACLCommand := []string{"--id=@acl", "create", "acl",
		...
		fmt.Sprintf("external-ids:default-deny-policy-type=%s", policyType)}
	if direction == fromLport {
		createACLCommand = append(createACLCommand, "options:apply-after-lb=true")
	}
	createACLCommand = append(createACLCommand, "--", "add", "port_group", portGroupUUID,
		"acls", "@acl")
...
func (oc *Controller) createDefaultDenyPortGroup(policyType knet.PolicyType) error {
	...
	err = addACLPortGroup(portGroupUUID, direction,
