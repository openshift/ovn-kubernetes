...
var _ = Describe("OVN Kubevirt Operations", func() {
	...
	var (
		...
		externalIDs = func(namespace, vmName, ovnZone string) map[string]string {
			...
			ids := map[string]string{
				...
				string(libovsdbops.NamespaceIndex):      namespace,
				string(libovsdbops.VirtualMachineIndex): vmName,
			}
			if vmName == kubevirt.AllVMs {
				delete(ids, string(libovsdbops.NamespaceIndex))
			}
			return ids
		}
		...
		composeDHCPv4Options = func(uuid, namespace string, t *testDHCPOptions) *nbdb.DHCPOptions {
			dhcpConfig := kubevirt.ComposeDHCPv4Config(
				t.cidr,
				t.dns,
			...
			dhcpConfig.Options.UUID = uuid

			return dhcpConfig.Options
		}
		composeDHCPv6Options = func(uuid, namespace string, t *testDHCPOptions) *nbdb.DHCPOptions {
			dhcpConfig := kubevirt.ComposeDHCPv6Config(
				t.cidr,
				t.dns,
				DefaultNetworkControllerName,
				namespace,
		...
		composeStaticRoute = func(uuid string, r testStaticRoute, t testData) *nbdb.LogicalRouterStaticRoute {
			...
			return &nbdb.LogicalRouterStaticRoute{
				...
				ExternalIDs: externalIDs(namespace, vmName, kubevirt.OvnLocalZone),
			}
		}
		...
		expectedNBDBAfterCleanup = func(expectedStaticRoutes []*nbdb.LogicalRouterStaticRoute) []libovsdb.TestData {
			...
			for _, nbData := range initialDB.NBData {
				// if there is no external ID they don't belong to kubevirt
				// and should be keep
				if policy, ok := nbData.(*nbdb.LogicalRouterPolicy); ok {
					...
					if staticRoute.ExternalIDs != nil && staticRoute.ExternalIDs[string(libovsdbops.VirtualMachineIndex)] != kubevirt.AllVMs {
						continue
					}
			...
			// resources for all VMs will not be removed
			for _, r := range expectedStaticRoutes {
				if r.ExternalIDs[string(libovsdbops.VirtualMachineIndex)] == kubevirt.AllVMs {
					expectedStaticRoutesAfterCleanup = append(expectedStaticRoutesAfterCleanup, r.UUID)
					data = append(data, r)
				}
			}
		...
		virtLauncher = func(suffix, node, vm, ipFamily string, extraLabels, extraAnnotations map[string]string) testVirtLauncherPod {
			var subnetIPv4, subnetIPv6, subnets, addressIPv4, addressIPv6, addresses, mac string
			if ipFamily == "ipv4" {
	...
	Context("during execution", func() {
		DescribeTable("reconcile migratable vm pods", func(t testData) {
			...
			if sourcePod != nil && t.migrationTarget.nodeName != "" {
			...
			PEntry("for pre-copy live migration at remote zone", testData{}),
			...
			PEntry("for pre-copy live migration to node owning subnet at local zone", testData{}),
			PEntry("for pre-copy live migration to node owning subnet at remote zone", testData{}),
