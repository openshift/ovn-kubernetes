...
var _ = Describe("OVN Kubevirt Operations", func() {
	...
	var (
		...
		composeStaticRoute = func(uuid string, r testStaticRoute, t testData) *nbdb.LogicalRouterStaticRoute {
			...
			ids := externalIDs(namespace, vmName, kubevirt.OvnLocalZone)
		...
		expectedNBDBAfterCleanup = func(expectedStaticRoutes []*nbdb.LogicalRouterStaticRoute) []libovsdb.TestData {
			...
			for _, nbData := range initialDB.NBData {
				...
				if policy, ok := nbData.(*nbdb.LogicalRouterPolicy); ok {
					if policy.ExternalIDs[string(libovsdbops.VirtualMachineIndex)] != "" {
						continue
					}
					...
					if staticRoute.ExternalIDs[string(libovsdbops.VirtualMachineIndex)] != "" {
						continue
					}
			...
			for _, expectedStaticRoute := range expectedStaticRoutes {
				...
				if expectedStaticRoute.ExternalIDs[string(libovsdbops.VirtualMachineIndex)] != "" {
					continue
				}
		...
		DescribeTable("reconcile migratable vm pods", func(t testData) {
			...
			Entry("for pre-copy live migration at local zone", testData{
				...
				expectedStaticRoutes: []testStaticRoute{
					...
					{
						prefix:  nodeByName[node1].subnetIPv4,
						nexthop: strings.Split(nodeByName[node1].transitSwitchPortIPv4, "/")[0],
					...
					{
						prefix:  nodeByName[node1].subnetIPv6,
						nexthop: strings.Split(nodeByName[node1].transitSwitchPortIPv6, "/")[0],
						policy:  &nbdb.LogicalRouterStaticRoutePolicySrcIP,
						externalIDs: map[string]string{
							kubevirt.OvnZoneExternalIDKey: kubevirt.OvnLocalZone,
						},
