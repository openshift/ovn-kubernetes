...
func getANPGressACL(action, anpName, direction string, rulePriority int32, ruleIndex int32, ports *[]anpapi.AdminNetworkPolicyPort, banp bool) []*nbdb.ACL {
	...
	if banp {
		...
		acl.ExternalIDs[libovsdbops.PrimaryIDKey.String()] = fmt.Sprintf("%s:BaselineAdminNetworkPolicy:%s:%s:%d",
	...
	if ports != nil && len(*ports) != 0 {
		...
		for i, port := range *ports {
			...
			if banp {
				aclCopy.ExternalIDs[libovsdbops.PrimaryIDKey.String()] = fmt.Sprintf("%s:BaselineAdminNetworkPolicy:%s:%s:%d.%d",
					DefaultNetworkControllerName, anpName, direction, ruleIndex, i)
				aclCopy.Name = utilpointer.String(fmt.Sprintf("BANP:%s:%s:%d.%d", anpName, direction, ruleIndex, i)) // tests logic for GetACLName
			}
