...
func (p PodAdmission) ValidateUpdate(ctx context.Context, oldObj, newObj runtime.Object) (err error) {
	...
	if oldPod.Spec.NodeName != nodeName {
		return fmt.Errorf("ovnkube-node on node: %q is not allowed to modify pods %q annotations", nodeName, oldPod.Name)
	}
	if newPod.Spec.NodeName != nodeName {
		return fmt.Errorf("ovnkube-node on node: %q is not allowed to modify pods %q annotations", nodeName, newPod.Name)
	}

	// ovnkube-node is not allowed to change annotations outside of it's scope
	if !p.annotationKeys.HasAll(changedKeys...) {
		return fmt.Errorf("ovnkube-node on node: %q is not allowed to set the following annotations on pod: %q: %v",
