...
func (n *NodeController) addOrUpdatePod(pod *kapi.Pod) error {
	...
	namespace, err := n.kube.KClient.CoreV1().Namespaces().Get(pod.Namespace, metav1.GetOptions{})
	if err != nil {
		klog.V(5).Infof("unable to get namespace %s annotations", pod.Namespace)
	}
	if namespace == nil {
		return nil
	}
	podExternalGW, ok := namespace.GetAnnotations()[hotypes.HybridOverlayExternalGw]
	if ok {
		_, _, err = util.RunOVSOfctl("add-flow", extBridgeName,
			fmt.Sprintf("table=0, cookie=0x%s, priority=100, ip, nw_src=%s, actions=load:%s->NXM_NX_TUN_ID[0..31],set_field:%s->tun_dst,output:%s", cookie, podIP.IP, strconv.Itoa(hotypes.HybridOverlayExternalGwVNI), podExternalGW, extVXLANName))
		if err != nil {
			return fmt.Errorf("failed to add flows for pod %s/%s: %v", pod.Namespace, pod.Name, err)
		}
		firstIP, secondIP := util.GetNodeWellKnownAddresses(subnet)
		...
		for _, sub := range config.Default.ClusterSubnets {
			route := util.PodRoute{
				Dest:    sub.CIDR,
				NextHop: firstIP.IP,
			}
		...
		if err = n.kube.SetAnnotationsOnPod(pod, marshalledAnnotation); err != nil {
