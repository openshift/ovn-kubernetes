...
func podToCookie(pod *kapi.Pod) string {
	return nodeNameToCookie(pod.Namespace + "_" + pod.Name)
}
...
func getPodDetails(pod *kapi.Pod, nodeName string) (string, string) {
	...
	return podInfo.IP.IP.String(), podInfo.MAC.String()
}
...
// nodeAddOrUpdate sets up or tears down VXLAN tunnels to Windows nodes in the
// cluster
func (n *NodeController) nodeAddOrUpdate(node *kapi.Node) error {
	...
	cidr, nodeIP, drMAC := getNodeDetails(node, houtil.IsWindowsNode, true)
	if cidr == nil || nodeIP == nil || drMAC == nil {
		return nil
	}

	// (re)add flows for the node
	cookie := nodeNameToCookie(node.Name)
...
func (n *NodeController) Update(oldNode, newNode *kapi.Node) {
	if nodeChanged(oldNode, newNode, houtil.IsWindowsNode) {
		if err := n.nodeAddOrUpdate(newNode); err != nil {
			logrus.Warning(err)
		}
	}
}
...
func (n *NodeController) Delete(node *kapi.Node) {
	...
	if err := deleteNodeFlowsByCookie(nodeNameToCookie(node.Name)); err != nil {
...
func (n *NodeController) initHybridOverlayBridge(nodeName string) error {
	...
	if err := n.kube.SetAnnotationOnNode(thisNode, types.HybridOverlayDrIP, portIP.String()); err != nil {
