...
func (n *NodeController) Add(node *kapi.Node) {
	...
	if node.Status.NodeInfo.MachineID != n.thisNode.Status.NodeInfo.MachineID {
		cidr, nodeIP, drMAC := getNodeDetails(node, nil, true)
		...
		cidr, nodeIP, _ := getNodeDetails(node, nil, false)
...
func (n *NodeController) Update(oldNode, newNode *kapi.Node) {
	...
	if nodeChanged(oldNode, newNode, nil) {
		n.Delete(oldNode)
		n.Add(newNode)
	}
}
...
func (n *NodeController) UninitSelf() {
	logrus.Infof("Removing overlay network '%s' (ID: %v)", networkName, n.networkID)
