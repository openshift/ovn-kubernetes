...
for crd in ${crds}; do
  ...
  # Clean up Kubernetes-generated files to avoid stale copies. Some CRD apis
  # trees also contain mockery-generated mocks used by unit tests; for example,
  # the VTEP informer and lister mocks live below this apis tree. Those mocks
  # are generated by make mocksgen, not by the Kubernetes generators below, so
  # preserve mocks/ directories while removing the generated client, lister,
  # informer, and applyconfiguration Go files.
  echo "Cleaning up existing generated files for $crd ($api_version)"
  apis_dir="${SCRIPT_ROOT}/pkg/crd/${crd}/${api_version}/apis"
  if [ -d "${apis_dir}" ]; then
    find "${apis_dir}" -mindepth 1 -type d -name mocks -prune -o -type f -name '*.go' -exec rm -f {} +
    find "${apis_dir}" -depth -type d -empty ! -path '*/mocks' ! -path '*/mocks/*' -exec rmdir {} +
  fi
