...
- **IPv6 (without static entry)**: the kernel sends a Neighbor
  ...
  `NORMAL` does **not** flood to NO_FLOOD CUDN ports, and unlike IPv4
  broadcast, the solicited-node multicast does not reach remote nodes
  that can respond. The CUDN GRs never see the NS, the kernel's
  neighbor entry enters `FAILED` state, and the traffic is dropped.
...
| Per-UDN | `V4MasqIPs.GatewayRouter`, `V6MasqIPs.GatewayRouter` | `addUDNMasqIPNeighbors()` in UDN `addNetwork()` | Not yet — only at network add time (startup/reconcile). Mid-run bridge recreation is a follow-up. |
