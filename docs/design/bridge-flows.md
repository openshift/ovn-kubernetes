...
use normal L2 forwarding (priority-0 NORMAL) unless addressed to the
bridge MAC, in which case the priority-10 fan-out replicates them to all
patch ports.
...
All table-0 ARP/NDP flows above are generated by `arpFanoutFilterFlows()` in
