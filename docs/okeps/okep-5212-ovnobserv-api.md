...
- with netobserv, samples may be displayed in a similar to observability use case way, but may need another interface to ensure
...
9. netobserv UI or ovnkube-observ script shows a sample with <`source pod A`, `dst_IP`> saying "Denied by network policy in namespace bigbank, direction Egress"
...
If 2 `ObservabilityConfigs` are using the same `CollectorID`, it is considered a misconfiguration, and only one of them
will be applied. Since the controller will likely be single-threaded, the first-handled `ObservabilityConfig` will
get the `ObservabilityStatus` condition `Applied` reported, and the seconds one will be rejected. This logic
also survives an ovn-kubernetes restart with the right initialSync.
