...
// getServerIP returns an external server container's IP address for the given IP family.
func getServerIP(serverName string, family utilnet.IPFamily) string {
	ginkgo.GinkgoHelper()
...
// getPodIP returns the pod's primary network IP address for the given IP family.
func getPodIP(k8sClient kubernetes.Interface, pod *v1.Pod, networkName string, family utilnet.IPFamily) string {
	ginkgo.GinkgoHelper()
	ip, err := getPodAnnotationIPsForPrimaryNetworkByIPFamily(
		k8sClient, pod.Namespace, pod.Name, networkName, family,
	)
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
...
// evpnBridgeName returns the Linux bridge name for an EVPN test, e.g. "brevpn7a3f".
// The "br" prefix keeps the worst-case SVI name ("brevpn9999.4094") at 15 chars
// (Linux IFNAMSIZ limit).
func evpnBridgeName(testName string) string { return "br" + testName }

// evpnVxlanName returns the VXLAN device name for an EVPN test, e.g. "vxevpn7a3f".
func evpnVxlanName(testName string) string { return "vx" + testName }

// evpnSVIName returns the SVI (VLAN sub-interface) name, e.g. "brevpn7a3f.100".
func evpnSVIName(bridgeName string, vid int) string {
	return fmt.Sprintf("%s.%d", bridgeName, vid)
}

// evpnIPVRFName returns the Linux VRF device name for a given IP-VRF VNI,
// e.g. "vrf20102".
func evpnIPVRFName(vni int32) string { return fmt.Sprintf("vrf%d", vni) }

// evpnMACVRFAgnhostName returns the Podman network/container name for the
// MAC-VRF agnhost server, e.g. "evpntest7a3f-macvrf-agnhost".
func evpnMACVRFAgnhostName(networkName string) string { return networkName + "-macvrf-agnhost" }

// evpnIPVRFAgnhostName returns the Podman network/container name for the
// IP-VRF agnhost server, e.g. "evpntest7a3f-ipvrf-agnhost".
func evpnIPVRFAgnhostName(networkName string) string { return networkName + "-ipvrf-agnhost" }
...
func setupSVIOnExternalFRR(ictx infraapi.Context, vid int, bridgeName, vrfName string) error {
	frr := infraapi.ExternalContainer{Name: externalFRRContainerName}
	vidStr := fmt.Sprintf("%d", vid)
	sviName := evpnSVIName(bridgeName, vid)
	...
	ictx.AddCleanUpFn(func() error {
		...
		if err != nil {
			if strings.Contains(err.Error(), "Cannot find device") {
				framework.Logf("SVI %s already absent on %s", sviName, externalFRRContainerName)
				return nil
			}
...
func setupEVPNBridgeOnExternalFRR(ictx infraapi.Context, frrVTEPIPAddress, bridgeName, vxlanName string) error {
	...
	ictx.AddCleanUpFn(func() error {
		...
		if err != nil {
			if strings.Contains(err.Error(), "Cannot find device") {
				framework.Logf("VXLAN %s already absent on %s", vxlanName, externalFRRContainerName)
			} else {
		...
		if err != nil {
			if strings.Contains(err.Error(), "Cannot find device") {
				framework.Logf("Bridge %s already absent on %s", bridgeName, externalFRRContainerName)
			} else {
...
// waitForExternalFRRDaemonsReady polls until both bgpd and zebra are listed in
// "show daemons", confirming they have restarted and reconnected to mgmtd after
// a daemon restart. "show daemons" only lists daemons that are up and connected;
// a daemon still starting is absent from the output even though vtysh (mgmtd)
// responds with exit code 0.
func waitForExternalFRRDaemonsReady(timeout time.Duration) error {
	frr := infraapi.ExternalContainer{Name: externalFRRContainerName}
	return wait.PollImmediate(3*time.Second, timeout, func() (bool, error) {
		out, err := infraprovider.Get().ExecExternalContainerCommand(frr, vtyshCommand("show daemons"))
		if err != nil {
			framework.Logf("FRR daemons not ready yet: %v", err)
			return false, nil
		}
		hasBGPD := strings.Contains(out, "bgpd")
		hasZebra := strings.Contains(out, "zebra")
		if !hasBGPD || !hasZebra {
			framework.Logf("FRR daemons not all ready yet (bgpd=%v zebra=%v): %q", hasBGPD, hasZebra, out)
			return false, nil
		}
		framework.Logf("FRR daemons ready (bgpd and zebra confirmed): %q", out)
		return true, nil
	})
}
