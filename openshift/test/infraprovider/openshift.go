...
type openshift struct {
	...
	host         *hypervisor
	hostNetworks map[string]*containerengine.Network
...
func New(config *rest.Config) (api.Provider, error) {
	...
	o := openshift{
		...
		nodes:        infraNodes,
		hostNetworks: map[string]*containerengine.Network{primaryNetworkName: primaryNet}}
	...
	if err != nil {
		...
		if _, err := host.execCmd("echo 'connection test'"); err != nil {
			...
			if err := host.findAndInitializeNetwork(primaryNetworkName, v4, v6); err != nil {
				...
				o.ContainerOps = &base.ContainerOps{Engine: containerengine.Podman, CmdRunner: host}
				o.host = host
...
func loadKubeNodes(kubeClient *kubernetes.Clientset) (map[string]*ocpNode, *containerengine.Network, error) {
	nodeMap := map[string]*ocpNode{}
	nodeList, err := kubeClient.CoreV1().Nodes().List(context.Background(), metav1.ListOptions{})
	if err != nil {
		return nil, nil, fmt.Errorf("failed to retrieve nodes from the cluster: %w", err)
	}
	primaryNet := &containerengine.Network{NetName: primaryNetworkName}
	for _, node := range nodeList.Items {
		...
		var cidrs []containerengine.NetworkConfig
		if ip4, ok := nodeIfAddr["ipv4"]; ok {
			...
			cidrs = append(cidrs, containerengine.NetworkConfig{Subnet: nodeNetInfo.v4Subnet})
		}
		if ip6, ok := nodeIfAddr["ipv6"]; ok {
			...
			cidrs = append(cidrs, containerengine.NetworkConfig{Subnet: nodeNetInfo.v6Subnet})
		}
