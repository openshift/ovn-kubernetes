...
// Run implements command.Runner run method to run podman specific
// commands over SSH.
func (r *hypervisor) Run(args ...string) (string, error) {
...
func (h *hypervisor) execCmd(cmdArgs []string) (result, error) {
	var r result
	hypervisorClient, err := h.getHypervisorClient()
	if err != nil {
		return r, err
	}
	r, err = runSSHCommand(hypervisorClient, cmdArgs)
	if err != nil {
...
func (h *hypervisor) findAndInitializeNetwork(name, v4Subnet, v6Subnet string) error {
	ipAddrCmdArgs := []string{"ip", "-j", "addr"}
	result, err := h.execCmd(ipAddrCmdArgs)
	if err != nil {
