...
var _ = g.Describe("[sig-network] SDN networkpolicy", func() {
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][WRS][V-BR.33] 41080-Check network policy ACL audit messages are logged to journald", g.Serial, func() {
		...
		cmd := "journalctl -t ovn-controller --since '1min ago' | grep 'verdict=allow' || true"
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 65901-Duplicate transactions should not be executed for network policy for every pod update.", g.Serial, func() {
		...
		getAddrSetUUIDCmd := fmt.Sprintf("ovn-nbctl --columns=_uuid --bare --no-leader-only find address_set 'external_ids:k8s.ovn.org/owner-type=PodSelector' 'external_ids:k8s.ovn.org/name=\"%s_LS{ML:{type: red,},}\"' 'addresses{>=}\"%s\"'", ns, podIP1)
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs][Disruptive] 64787-Network policy with duplicate egress rules (same CIDR block) fails to be recreated", g.Serial, func() {
		var (
			buildPruningBaseDir         = testdata.FixturePath("networking")
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs][Disruptive] 64786-Network policy in namespace that has long name fails to be recreated as the ACLs are considered duplicate", g.Serial, func() {
		var (
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs][Disruptive] 64788-Same network policies across multiple namespaces fail to be recreated.", g.Serial, func() {
		if oteutils.IsHypershiftHostedCluster(oc) {
			g.Skip("Not applicable to HyperShift hosted cluster")
		}
		var (
