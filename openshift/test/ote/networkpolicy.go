...
var _ = g.Describe("[sig-network] SDN networkpolicy", func() {
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 64787-Network policy with duplicate egress rules (same CIDR block) fails to be recreated", g.Label("Disruptive"), func() {
		var (
			buildPruningBaseDir         = testdata.FixturePath("networking")
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 64786-Network policy in namespace that has long name fails to be recreated as the ACLs are considered duplicate", g.Label("Disruptive"), func() {
		var (
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 64788-Same network policies across multiple namespaces fail to be recreated.", g.Label("Disruptive"), func() {
		if oteutils.IsHypershiftHostedCluster(oc) {
			g.Skip("Not applicable to HyperShift hosted cluster")
		}
		var (
