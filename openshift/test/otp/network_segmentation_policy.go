...
var _ = g.Describe("[sig-network][Suite:openshift/conformance/serial] SDN network segmentation policy", func() {
	...
	g.It("[JIRA:Networking][OTP] 75624-Feature integration UDN with multinetworkpolicy [Disruptive]", func() {
		var (
			testDataDirUDN         = testdata.FixturePath("networking/network_segmentation/udn")
		...
		ns1 := "project75624"
		defer oc.AsAdmin().WithoutNamespace().Run("delete").Args("namespace", ns1, "--ignore-not-found").Execute()
		...
		defer func() {
			oc.Run("config").Args("set-context", "--current", "--namespace="+origNs).Execute()
		}()
