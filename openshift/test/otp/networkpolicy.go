...
var _ = g.Describe("[sig-networking] SDN networkpolicy", func() {
	...
	g.It("[OTP] 49076-service domain can be resolved when egress type is enabled", func() {
		var (
	...
	g.It("[OTP] 49186-Networkpolicy egress rule should work for statefulset pods.", func() {
		var (
			buildPruningBaseDir  = testdata.FixturePath("networking")
			testPodFile          = filepath.Join(buildPruningBaseDir, "testpod.yaml")
	...
	g.It("[OTP] 49437-Ingress network policy shouldn't be overruled by egress network policy on another pod", func() {
		var (
			buildPruningBaseDir = testdata.FixturePath("networking")
	...
	g.It("[OTP] 49686-network policy with ingress rule with ipBlock", func() {
		var (
			buildPruningBaseDir          = testdata.FixturePath("networking")
	...
	g.It("[OTP] 49696-mixed ingress and egress policies can work well", g.Label("Level0"), func() {
		var (
			buildPruningBaseDir = testdata.FixturePath("networking")
			testPodFile         = filepath.Join(buildPruningBaseDir, "testpod.yaml")
			helloSdnFile        = filepath.Join(buildPruningBaseDir, "hellosdn.yaml")
	...
	g.It("[OTP] 46246-Network Policies should work with OVNKubernetes when traffic hairpins back to the same source through a service", func() {
		var (
			buildPruningBaseDir    = testdata.FixturePath("networking")
			pingPodNodeTemplate    = filepath.Join(buildPruningBaseDir, "ping-for-pod-specific-node-template.yaml")
	...
	g.It("[OTP] 41879-ipBlock should not ignore all other cidr's apart from the last one specified", func() {
		var (
			buildPruningBaseDir          = testdata.FixturePath("networking")
	...
	g.It("[OTP] 46807-network policy with egress rule with ipBlock", func() {
		var (
			buildPruningBaseDir         = testdata.FixturePath("networking")
	...
	g.It("[OTP] 46808-network policy with egress rule with ipBlock and except", func() {
		var (
			buildPruningBaseDir         = testdata.FixturePath("networking")
	...
	g.It("[OTP] 41082-Check ACL audit logs can be extracted", func() {
		var (
			buildPruningBaseDir = testdata.FixturePath("networking")
			allowFromSameNS     = filepath.Join(buildPruningBaseDir, "networkpolicy/allow-from-same-namespace.yaml")
			ingressTypeFile     = filepath.Join(buildPruningBaseDir, "networkpolicy/default-deny-ingress.yaml")
	...
	g.It("[OTP] 41407-Check networkpolicy ACL audit message is logged with correct policy name", func() {
		var (
			buildPruningBaseDir = testdata.FixturePath("networking")
			allowFromSameNS     = filepath.Join(buildPruningBaseDir, "networkpolicy/allow-from-same-namespace.yaml")
			ingressTypeFile     = filepath.Join(buildPruningBaseDir, "networkpolicy/default-deny-ingress.yaml")
	...
	g.It("[OTP] 41080-Check network policy ACL audit messages are logged to journald", func() {
		var (
			buildPruningBaseDir = testdata.FixturePath("networking")
			allowFromSameNS     = filepath.Join(buildPruningBaseDir, "networkpolicy/allow-from-same-namespace.yaml")
			ingressTypeFile     = filepath.Join(buildPruningBaseDir, "networkpolicy/default-deny-ingress.yaml")
	...
	g.It("[OTP] 55287-Default network policy ACLs to a namespace should not be present with arp but arp||nd for ARPAllowPolicies", func() {
		var (
			buildPruningBaseDir = testdata.FixturePath("networking")
			testPodFile         = filepath.Join(buildPruningBaseDir, "testpod.yaml")
			ingressTypeFile     = filepath.Join(buildPruningBaseDir, "networkpolicy/default-deny-ingress.yaml")
		...
		listOutput, listErr := otputils.RemoteShPodWithBash(oc, "openshift-ovn-kubernetes", ovnMasterPodName, listACLCmd)
	...
	g.It("[OTP] 62524-OVN address_set referenced in acl should not miss when networkpolicy name includes dot.", func() {
		...
		for _, addrSetName := range result {
			...
			listAddrOutput, listAddrErr := otputils.RemoteShPodWithBash(oc, "openshift-ovn-kubernetes", ovnMasterPodName, listAddressSetCmd)
	...
	g.It("[OTP] 65901-Duplicate transactions should not be executed for network policy for every pod update.", func() {
	...
	g.It("[OTP] 66085-Creating egress network policies for allowing to same namespace and openshift dns in namespace prevents the pod from reaching its own service", func() {
	...
	g.It("[OTP] 64787-Network policy with duplicate egress rules (same CIDR block) fails to be recreated", g.Label("Disruptive"), func() {
	...
	g.It("[OTP] 64786-Network policy in namespace that has long name fails to be recreated as the ACLs are considered duplicate", g.Label("Disruptive"), func() {
	...
	g.It("[OTP] 64788-Same network policies across multiple namespaces fail to be recreated .", g.Label("Disruptive"), func() {
	...
	g.It("[OTP] 68660-Exposed route of the service should be accessible when allowing inbound traffic from any namespace network policy is created.", func() {
	...
	g.It("[OTP] 75540-Network Policy Validation", func() {
		var (
			buildPruningBaseDir = testdata.FixturePath("networking")
			networkPolicyFile   = filepath.Join(buildPruningBaseDir, "networkpolicy/netpol-30920-75540.yaml")
		)
		g.By("OCPBUGS-30920 Verify the network policy is not created with invalid value")
		oc.SetupProject()
		ns := oc.Namespace()
		o.Expect(otputils.CreateResourceFromFileWithError(oc, ns, networkPolicyFile)).To(o.HaveOccurred())
	})

	g.It("[OTP] 70009-Pod IP is missing from OVN DB AddressSet when using allow-namespace-only network policy", func() {
		var (
			buildPruningBaseDir          = testdata.FixturePath("networking")
		...
		listPGCOutput, listErr := otputils.RemoteShPodWithBash(oc, "openshift-ovn-kubernetes", ovnNodePod, listPGCmd)
		o.Expect(listErr).NotTo(o.HaveOccurred())
		...
		listAclOutput, listErr := otputils.RemoteShPodWithBash(oc, "openshift-ovn-kubernetes", ovnNodePod, listAclCmd)
		o.Expect(listErr).NotTo(o.HaveOccurred())
		o.Expect(listAclOutput).NotTo(o.BeEmpty())
		...
		for _, addrSetName := range trimmedAddrSetNames {
			...
			listAddrOutput, listErr := otputils.RemoteShPodWithBash(oc, "openshift-ovn-kubernetes", ovnNodePod, listAddressSetCmd)
			o.Expect(listErr).NotTo(o.HaveOccurred())
			o.Expect(listAddrOutput).NotTo(o.BeEmpty())
	...
	g.It("[OTP] 83735-Verify NP with label npprotection/blockdeletion=true label can not be deleted after VAP is applied.", func() {
		var (
			buildPruningBaseDir = testdata.FixturePath("networking")
...
var _ = g.Describe("[sig-networking] SDN networkpolicy", func() {
	...
	g.It("[OTP] 69234-high memory usage on ovnkube-master leader pods on some clusters when a network policy is deleted.", g.Serial, func() {
		var (
			buildPruningBaseDir           = testdata.FixturePath("networking")
