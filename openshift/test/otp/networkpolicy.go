...
var _ = g.Describe("[sig-network] SDN networkpolicy", func() {
	...
	g.It("[JIRA:Networking][OTP][FdpOvnOvs] 49696-mixed ingress and egress policies can work well", func() {
		var (
			buildPruningBaseDir = testdata.FixturePath("networking")
			testPodFile         = filepath.Join(buildPruningBaseDir, "testpod.yaml")
			helloSdnFile        = filepath.Join(buildPruningBaseDir, "hellosdn.yaml")
...
var _ = g.Describe("[sig-network] SDN networkpolicy", func() {
	...
	g.It("[JIRA:Networking][OTP][Serial] 69234-high memory usage on ovnkube-master leader pods on some clusters when a network policy is deleted.", func() {
		var (
			buildPruningBaseDir           = testdata.FixturePath("networking")
