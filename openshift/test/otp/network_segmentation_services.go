...
var _ = g.Describe("[sig-network] SDN network segmentation services", func() {
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 76017-Service should be able to access for same NAD UDN pods in different namespaces (L3/L2)", func() {
		var (
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 76732-Validate pod2Service/nodePortService for UDN(Layer2)", func() {
		var (
			buildPruningBaseDir    = testdata.FixturePath("networking")
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 75942-Validate pod2Service/nodePortService for UDN(Layer3)", func() {
		var (
			buildPruningBaseDir    = testdata.FixturePath("networking")
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 76014-Validate LoadBalancer service for UDN pods (Layer3/Layer2)", func() {
		buildPruningBaseDir := testdata.FixturePath("networking")
		udnPodTemplate := filepath.Join(testDataDirUDN, "udn_test_pod_template.yaml")
		genericServiceTemplate := filepath.Join(buildPruningBaseDir, "service-generic-template.yaml")
		udnCRDSingleStack := filepath.Join(testDataDirUDN, "udn_crd_singlestack_template.yaml")
		...
		for i := 0; i < 2; i++ {
			...
			err = wait.PollUntilContextTimeout(context.TODO(), 10*time.Second, 120*time.Second, false, func(_ context.Context) (bool, error) {
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 78767-Validate service for CUDN(Layer3)", func() {
		var (
			buildPruningBaseDir    = testdata.FixturePath("networking")
			pingPodTemplate        = filepath.Join(buildPruningBaseDir, "ping-for-pod-specific-node-template.yaml")
			genericServiceTemplate = filepath.Join(buildPruningBaseDir, "service-generic-template.yaml")
		...
		for i := 0; i < 2; i++ {
			...
			defer func() {
				_ = oc.AsAdmin().WithoutNamespace().Run("label").Args("ns", cudnNS[i], fmt.Sprintf("%s-", key)).Execute()
			}()
		...
		defer func() {
			_ = oc.AsAdmin().WithoutNamespace().Run("label").Args("ns", cudnNS[3], fmt.Sprintf("%s-", key)).Execute()
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 78768-Validate service for CUDN(Layer2)", func() {
		var (
			buildPruningBaseDir    = testdata.FixturePath("networking")
			pingPodTemplate        = filepath.Join(buildPruningBaseDir, "ping-for-pod-specific-node-template.yaml")
			genericServiceTemplate = filepath.Join(buildPruningBaseDir, "service-generic-template.yaml")
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 44790-Validate ExternalIP service for default and UDN pods - setup", g.Serial, func() {
		nodeList, nodeErr := e2enode.GetReadySchedulableNodes(context.TODO(), oc.KubeFramework().ClientSet)
		o.Expect(nodeErr).NotTo(o.HaveOccurred())
		if len(nodeList.Items) < 3 {
			g.Skip("This test requires at least 3 worker nodes which is not fulfilled. ")
		...
		_ = oc.AsAdmin().WithoutNamespace().Run("create").Args("namespace", allNS[2]).Execute()
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 79163-Validate ExternalIP service for default and UDN pods - verify", g.Serial, func() {
		...
		for i := 0; i < 3; i++ {
			defer func() {
				_ = oc.AsAdmin().WithoutNamespace().Run("delete").Args("project", allNS[i], "--ignore-not-found=true").Execute()
			}()
		}
