...
var _ = g.Describe("[sig-network] SDN network segmentation", func() {
	...
	waitAllPodsReady := func(oc *exutil.CLI, namespace string) {
		err := wait.PollUntilContextTimeout(context.TODO(), 10*time.Second, 4*time.Minute, true, func(_ context.Context) (bool, error) {
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 75223-Restarting ovn pods should not break UDN primary network traffic", g.Serial, func() {
		var (
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 75254-Check kubelet probes are allowed via default network LSP for the UDN pods", func() {
		var (
		...
		defer func() { _ = cmdTcpdump1.Process.Kill() }()
		...
		defer func() { _ = cmdTcpdump2.Process.Kill() }()
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 75503-Overlapping pod CIDRs IPs are allowed in different primary NADs", func() {
		var (
			testDataDir    = testdata.FixturePath("networking")
			udnNadtemplate = filepath.Join(testDataDirUDN, "udn_nad_template.yaml")
			testPodFile    = filepath.Join(testDataDir, "testpod.yaml")
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 75955-Verify UDN failed message when user defined join subnet overlaps user defined subnet Layer3", func() {
		var (
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 75984-Check udn pods isolation on user defined networks post OVN gateway migration", g.Serial, func() {
		var (
			udnNadtemplate = filepath.Join(testDataDirUDN, "udn_nad_template.yaml")
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 76939-Check udn pods isolation on a scaled node", g.Serial, func() {
		var (
			udnPodTemplate     = filepath.Join(testDataDirUDN, "udn_test_pod_template.yaml")
		...
		defer func() {
			...
			_ = wait.PollUntilContextTimeout(context.TODO(), 30*time.Second, 15*time.Minute, true, func(_ context.Context) (bool, error) {
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 77542-Check default network ports can be exposed on UDN pods layer3", g.Serial, func() {
		var (
			testDataDir         = testdata.FixturePath("networking")
			netsegDir           = testdata.FixturePath("networking/network_segmentation")
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 77742-Check default network ports can be exposed on UDN pods layer2", g.Serial, func() {
		var (
			testDataDir         = testdata.FixturePath("networking")
			netsegDir           = testdata.FixturePath("networking/network_segmentation")
			sctpModule          = filepath.Join(netsegDir, "sctp/load-sctp-module.yaml")
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 78152-Check udn pods to kapi dns traffic should pass", func() {
		var (
			testDataDir     = testdata.FixturePath("networking")
		...
		defer func() {
			_ = oc.AsAdmin().WithoutNamespace().Run("delete").Args("svc", testSvc.Servicename, "-n", testSvc.Namespace).Execute()
		}()
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 78381-Check cudn pods to kapi dns traffic layer 2", func() {
		var (
			testDataDir     = testdata.FixturePath("networking")
			testPodTemplate = filepath.Join(testDataDir, "ping-for-pod-template.yaml")
			serviceTemplate = filepath.Join(testDataDir, "service-generic-template.yaml")
		...
		defer func() {
			_ = oc.AsAdmin().WithoutNamespace().Run("label").Args("ns", ns, fmt.Sprintf("%s-", key)).Execute()
		}()
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 79095-Verify event is generated for IP exhaustion in user defined network", func() {
		var (
