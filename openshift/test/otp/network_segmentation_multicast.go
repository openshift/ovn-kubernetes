...
var _ = g.Describe("[sig-network] SDN network segmentation multicast", func() {
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 78447-udn pods should/should not receive multicast traffic when enable/disable multicast.", g.Serial, func() {
		var (
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 78448-udn pods can join different multicast groups at same time.", g.Serial, func() {
		var (
			mcastPodTemplate = filepath.Join(testDataDirMcast, "multicast-rc.json")
			ns               string
			ipStackType      = otputils.CheckIPStackType(oc)
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 78450-Same multicast groups can be created in multiple namespaces with udn configured.", g.Serial, func() {
		var (
			mcastPodTemplate = filepath.Join(testDataDirMcast, "multicast-rc.json")
			ns1              string
			ns2              string
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][Feature:NetworkSegmentation] 78382-check CUDN pods should not be able to receive multicast traffic from other pods in different namespace which sharing a same CUDN (layer 3).", g.Serial, func() {
		var (
			mcastPodTemplate = filepath.Join(testDataDirMcast, "multicast-rc.json")
			ns1              string
			ns2              string
		...
		for i := 0; i < 2; i++ {
			if i == 0 {
				...
				defer func() {
					_ = oc.AsAdmin().WithoutNamespace().Run("label").Args("ns", ns1, fmt.Sprintf("%s-", key)).Execute()
				}()
				err := oc.AsAdmin().WithoutNamespace().Run("label").Args("ns", ns1, fmt.Sprintf("%s=%s", key, values[0])).Execute()
				o.Expect(err).NotTo(o.HaveOccurred())

				defer func() {
					_ = oc.AsAdmin().WithoutNamespace().Run("label").Args("ns", ns2, fmt.Sprintf("%s-", key)).Execute()
				}()
