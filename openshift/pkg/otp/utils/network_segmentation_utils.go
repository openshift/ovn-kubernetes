...
func (udncrd *UdnCRDResource) CreateUdnCRDSingleStack(oc *exutil.CLI) {
	err := wait.PollUntilContextTimeout(context.TODO(), 2*time.Second, 10*time.Second, false, func(_ context.Context) (bool, error) {
...
func VerifyConnPod2Pod(oc *exutil.CLI, namespaceSrc string, podNameSrc string, namespaceDst string, podNameDst string, protocol string, port int, pass bool) {
	...
	for _, nsPod := range [][]string{{namespaceSrc, podNameSrc}, {namespaceDst, podNameDst}} {
		_, _ = e2eoutput.RunHostCmd(nsPod[0], nsPod[1], "killall socat")
	}
	...
	if pass {
		if podIP2 != "" {
			...
			o.Expect(strings.Count(serverCmdOutput.String(), "hello")).To(o.Equal(2))
		} else {
		...
		if podIP2 != "" {
			clientCmd := fmt.Sprintf("timeout 10 sh -c 'echo hello | socat - %s:%s'", clientOpt, net.JoinHostPort(podIP1, strconv.Itoa(port)))
			_, _ = e2eoutput.RunHostCmd(namespaceSrc, podNameSrc, clientCmd)
...
func StartMcastTrafficOnPod(_ *exutil.CLI, ns string, pod string, ipList []string, pktfile string, mcastip string, port string) {
	ipStr := strings.Join(ipList, " ")
	if port == "" {
		port = "4321"
	}
	go func() {
		ompingCmd := "omping " + "-q " + "-p " + port + " -c 20 -T 20 -m " + mcastip + " " + ipStr + " > " + pktfile + " &"
...
func ChkMcatRcvOnPod(_ *exutil.CLI, ns string, pod string, podip string, iplist []string, mcastip string, pktfile string) bool {
	catCmd := "cat " + pktfile
...
func ChkMcastAddress(_ *exutil.CLI, ns string, pod string, intf string, mcastip string) {
...
// DualstackNAD represents a dual-stack NetworkAttachmentDefinition resource
...
func (nad *DualstackNAD) CreateDualstackNAD(oc *exutil.CLI) {
	err := wait.PollUntilContextTimeout(context.TODO(), 5*time.Second, 20*time.Second, true, func(_ context.Context) (bool, error) {
		err1 := ApplyResourceFromTemplateByAdmin(oc, "--ignore-unknown-parameters=true", "-f", nad.Template, "-p",
			"NADNAME="+nad.Nadname, "NAMESPACE="+nad.Namespace, "PLUGINTYPE="+nad.Plugintype,
			"MODE="+nad.Mode, "IPAMTYPE="+nad.Ipamtype, "IPV4RANGE="+nad.Ipv4range,
			"IPV6RANGE="+nad.Ipv6range, "IPV4RANGESTART="+nad.Ipv4rangestart,
			"IPV4RANGEEND="+nad.Ipv4rangeend, "IPV6RANGESTART="+nad.Ipv6rangestart,
			"IPV6RANGEEND="+nad.Ipv6rangeend)
		if err1 != nil {
			e2e.Logf("Error creating dualstack NAD: %v, retrying", err1)
			return false, nil
		}
		return true, nil
	})
	o.Expect(err).NotTo(o.HaveOccurred(), fmt.Sprintf("Failed to create dualstack NAD %v", nad.Nadname))
}

// TestMultihomingPod represents a pod attached to multiple networks
...
func (pod *TestMultihomingPod) CreateTestMultihomingPod(oc *exutil.CLI) {
	err := wait.PollUntilContextTimeout(context.TODO(), 5*time.Second, 20*time.Second, true, func(_ context.Context) (bool, error) {
		err1 := ApplyResourceFromTemplateByAdmin(oc, "--ignore-unknown-parameters=true", "-f", pod.Template, "-p",
			"NAME="+pod.Name, "NAMESPACE="+pod.Namespace, "PODLABEL="+pod.Podlabel,
			"NADNAME="+pod.Nadname, "PODENVNAME="+pod.Podenvname, "NODENAME="+pod.Nodename)
		if err1 != nil {
			e2e.Logf("Error creating multihoming pod: %v, retrying", err1)
			return false, nil
		}
		return true, nil
	})
	o.Expect(err).NotTo(o.HaveOccurred(), fmt.Sprintf("Failed to create multihoming pod %v", pod.Name))
}

// GetPodMultiNetwork returns the IPv4 and IPv6 addresses of a pod's net1 (secondary) interface
func GetPodMultiNetwork(namespace string, podName string) (string, string) {
	...
	o.Expect(err).NotTo(o.HaveOccurred())
	podIPv6, err := e2eoutput.RunHostCmdWithRetries(namespace, podName, cmd2, 2*time.Second, 10*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	return strings.TrimSpace(podIPv4), strings.TrimSpace(podIPv6)
}

// CurlPod2PodMultiNetworkPass validates that curl to a pod's secondary network IPs succeeds
func CurlPod2PodMultiNetworkPass(namespaceSrc string, podNameSrc string, podIPv4 string, podIPv6 string) {
	err := wait.PollUntilContextTimeout(context.TODO(), 2*time.Second, 30*time.Second, false, func(_ context.Context) (bool, error) {
		...
		if !strings.Contains(msg, "Hello OpenShift!") {
			e2e.Logf("IPv4 curl should pass but failed, retrying")
			return false, nil
		}
		return true, nil
	})
	o.Expect(err).NotTo(o.HaveOccurred(), "IPv4 curl to secondary network should pass")

	err = wait.PollUntilContextTimeout(context.TODO(), 2*time.Second, 30*time.Second, false, func(_ context.Context) (bool, error) {
		msg, _ := e2eoutput.RunHostCmd(namespaceSrc, podNameSrc, "curl -g -6 ["+podIPv6+"]:8080 --connect-timeout 5")
		if !strings.Contains(msg, "Hello OpenShift!") {
			e2e.Logf("IPv6 curl should pass but failed, retrying")
			return false, nil
		}
		return true, nil
	})
	o.Expect(err).NotTo(o.HaveOccurred(), "IPv6 curl to secondary network should pass")
}

// CurlPod2PodMultiNetworkFail validates that curl to a pod's secondary network IPs fails
func CurlPod2PodMultiNetworkFail(namespaceSrc string, podNameSrc string, podIPv4 string, podIPv6 string) {
	err := wait.PollUntilContextTimeout(context.TODO(), 2*time.Second, 30*time.Second, false, func(_ context.Context) (bool, error) {
		msg, _ := e2eoutput.RunHostCmd(namespaceSrc, podNameSrc, "curl "+podIPv4+":8080 --connect-timeout 5")
		if strings.Contains(msg, "Hello OpenShift!") {
			e2e.Logf("IPv4 curl should fail but passed, retrying")
			return false, nil
		}
		return true, nil
	})
	o.Expect(err).NotTo(o.HaveOccurred(), "IPv4 curl to secondary network should fail")

	err = wait.PollUntilContextTimeout(context.TODO(), 2*time.Second, 30*time.Second, false, func(_ context.Context) (bool, error) {
		msg, _ := e2eoutput.RunHostCmd(namespaceSrc, podNameSrc, "curl -g -6 ["+podIPv6+"]:8080 --connect-timeout 5")
		if strings.Contains(msg, "Hello OpenShift!") {
			e2e.Logf("IPv6 curl should fail but passed, retrying")
			return false, nil
		}
		return true, nil
	})
	o.Expect(err).NotTo(o.HaveOccurred(), "IPv6 curl to secondary network should fail")
}
