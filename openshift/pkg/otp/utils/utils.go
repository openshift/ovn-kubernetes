...
func ApplyResourceFromTemplate(oc *exutil.CLI, parameters ...string) error {
	...
	err := wait.PollUntilContextTimeout(context.TODO(), 3*time.Second, 60*time.Second, true, func(_ context.Context) (bool, error) {
		...
		tmpFile, err := os.CreateTemp("", GetRandomString()+"ping-pod-*.json")
		if err != nil {
	...
	applyArgs := []string{"-f", configFile}
	for i, p := range parameters {
		if p == "-n" && i+1 < len(parameters) {
			applyArgs = append(applyArgs, "-n", parameters[i+1])
			break
		}
	}
	return oc.WithoutNamespace().Run("apply").Args(applyArgs...).Execute()
}
...
func ApplyResourceFromTemplateByAdmin(oc *exutil.CLI, parameters ...string) error {
	...
	return oc.WithoutNamespace().AsAdmin().Run("apply").Args(applyArgs...).Execute()
}
...
func DescribePod(oc *exutil.CLI, namespace string, podName string) string {
	podDescribe, err := oc.AsAdmin().WithoutNamespace().Run("describe").Args("pod", "-n", namespace, podName).Output()
	o.Expect(err).NotTo(o.HaveOccurred())
...
func CheckSctpModule(oc *exutil.CLI, nodeName, namespace string) {
	...
	err := wait.PollUntilContextTimeout(context.TODO(), 30*time.Second, 15*time.Minute, true, func(_ context.Context) (bool, error) {
...
func WaitForPodWithLabelGone(oc *exutil.CLI, ns, label string) error {
	errWait := wait.PollUntilContextTimeout(context.TODO(), 5*time.Second, 10*time.Minute, true, func(_ context.Context) (bool, error) {
...
func CheckNetworkOperatorState(oc *exutil.CLI, interval int, timeout int) {
	errCheck := wait.PollUntilContextTimeout(context.TODO(), time.Duration(interval)*time.Second, time.Duration(timeout)*time.Second, true, func(_ context.Context) (bool, error) {
...
func GetNodeIPv4(oc *exutil.CLI, _, nodeName string) string {
...
func GetLeaderInfo(oc *exutil.CLI, namespace string, cmName string, networkType string) string {
	...
	_ = json.Unmarshal([]byte(output), &sdnAnnotations)
	...
	defer func() { _ = os.RemoveAll(ocGetPods) }()
...
func CheckSDNMetrics(oc *exutil.CLI, url string, metrics string) {
	...
	metricsErr := wait.PollUntilContextTimeout(context.TODO(), 5*time.Second, 10*time.Second, true, func(_ context.Context) (bool, error) {
		output, err := oc.AsAdmin().WithoutNamespace().Run("exec").Args("-n", "openshift-monitoring", "-c", "prometheus", "prometheus-k8s-0", "--", "curl", "-k", "-H", fmt.Sprintf("Authorization: Bearer %v", olmToken), url).OutputToFile("metrics.txt")
		if err != nil {
...
func GetEgressIPByKind(oc *exutil.CLI, kind string, kindName string, expectedNum int) ([]string, error) {
	...
	if expectedNum == 0 {
		...
		egressIPEmptyErr := wait.PollUntilContextTimeout(context.TODO(), 30*time.Second, 5*time.Minute, true, func(_ context.Context) (bool, error) {
			iplist, err = oc.AsAdmin().WithoutNamespace().Run("get").Args(kind, kindName, "-o=jsonpath={.egressIPs}").Output()
			if iplist == "" || iplist == "[]" {
	...
	if isIPListEmpty || len(ip) < expectedNum || err != nil {
		err = wait.PollUntilContextTimeout(context.TODO(), 30*time.Second, 5*time.Minute, true, func(_ context.Context) (bool, error) {
			iplist, err = oc.AsAdmin().WithoutNamespace().Run("get").Args(kind, kindName, "-o=jsonpath={.egressIPs}").Output()
...
func FindTwoNodesWithSameSubnet(oc *exutil.CLI, nodeList *corev1.NodeList) (bool, [2]string) {
	var nodes [2]string
...
func GetSDNMetrics(oc *exutil.CLI, podName string) string {
	...
	metricsErr := wait.PollUntilContextTimeout(context.TODO(), 5*time.Second, 10*time.Second, true, func(_ context.Context) (bool, error) {
		output, err := oc.AsAdmin().WithoutNamespace().Run("exec").Args("-n", "openshift-sdn", podName, "--", "curl", "localhost:29100/metrics").OutputToFile("metrics.txt")
		if err != nil {
			e2e.Logf("Can't get metrics and try again, the error is:%s", err)
			return false, nil
		}
...
func GetOVNMetrics(oc *exutil.CLI, url string) string {
	...
	o.Expect(olmToken).NotTo(o.BeEmpty())
	metricsErr := wait.PollUntilContextTimeout(context.TODO(), 5*time.Second, 10*time.Second, true, func(_ context.Context) (bool, error) {
		output, err := oc.AsAdmin().WithoutNamespace().Run("exec").Args("-n", "openshift-monitoring", "-c", "prometheus", "prometheus-k8s-0", "--", "env", "-u", "HTTPS_PROXY", "-u", "https_proxy", "curl", "-k", "-H", fmt.Sprintf("Authorization: Bearer %v", olmToken), url).OutputToFile("metrics.txt")
		if err != nil {
			e2e.Logf("Can't get metrics and try again, the error is:%s", err)
			return false, nil
		}
...
func GetAssignedEIPInEIPObject(oc *exutil.CLI, egressIPObject string) []map[string]string {
	...
	egressipErr := wait.PollUntilContextTimeout(context.TODO(), 10*time.Second, timeout, true, func(_ context.Context) (bool, error) {
	...
	_ = json.Unmarshal([]byte(egressIPs), &egressIPJsonMap)
...
func CheckNodeStatus(oc *exutil.CLI, nodeName string, expectedStatus string) {
	...
	default:
		err1 := fmt.Errorf("unsupported node status")
		o.Expect(err1).NotTo(o.HaveOccurred())
	}
	err := wait.PollUntilContextTimeout(context.TODO(), 5*time.Second, 15*time.Minute, true, func(_ context.Context) (bool, error) {
...
func UpdateEgressIPObject(oc *exutil.CLI, egressIPObjectName string, egressIP string) {
	...
	egressipErr := wait.PollUntilContextTimeout(context.TODO(), 10*time.Second, 180*time.Second, true, func(_ context.Context) (bool, error) {
...
func GetTwoNodesSameSubnet(oc *exutil.CLI, nodeList *corev1.NodeList) (bool, []string) {
...
func SDNHostwEgressIP(oc *exutil.CLI, node []string, egressip string) string {
	...
	for i := 0; i < len(node); i++ {
		...
		if iplist == "" || iplist == "[]" || err != nil {
			err = wait.PollUntilContextTimeout(context.TODO(), 30*time.Second, 3*time.Minute, true, func(_ context.Context) (bool, error) {
				iplist, err = oc.AsAdmin().WithoutNamespace().Run("get").Args("hostsubnet", node[i], "-o=jsonpath={.egressIPs}").Output()
				if iplist != "" && iplist != "[]" {
...
func CheckPrimaryNIC(oc *exutil.CLI, nodeName string, ip string, flag bool) {
	checkErr := wait.PollUntilContextTimeout(context.TODO(), 10*time.Second, 60*time.Second, true, func(_ context.Context) (bool, error) {
...
func GetNodeMacAddress(oc *exutil.CLI, nodeName string) string {
	...
	var data map[string]interface{}
	_ = json.Unmarshal([]byte(output), &data)
	l3GatewayConfigAnnotationsJSON := data["default"].(map[string]interface{})
...
func SearchOVNDBForSpecCmd(oc *exutil.CLI, cmd, searchKeyword string, times int) error {
	...
	checkOVNDbErr := wait.PollUntilContextTimeout(context.TODO(), 10*time.Second, 2*time.Minute, true, func(_ context.Context) (bool, error) {
...
func PrepareSCTPModule(oc *exutil.CLI, _ string) {
...
func CheckClusterStatus(oc *exutil.CLI, _ string) {
...
func GetOVNKCtrlPlanePodOnHostedCluster(oc *exutil.CLI, _, _, _ string) string {
...
func WaitForPodWithLabelReadyOnHostedCluster(oc *exutil.CLI, ns, label string) error {
	return wait.PollUntilContextTimeout(context.TODO(), 15*time.Second, 10*time.Minute, true, func(_ context.Context) (bool, error) {
...
func GetReadySchedulableNodesOnHostedCluster(oc *exutil.CLI) ([]string, error) {
	...
	for _, nodeName := range nodesOnHostedCluster {
		err := wait.PollUntilContextTimeout(context.TODO(), 10*time.Second, 15*time.Minute, true, func(_ context.Context) (bool, error) {
...
func GetHostsubnetByEIP(oc *exutil.CLI, expectedEIP string) string {
	...
	for i, v := range nodeList.Items {
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		if ip[0] == expectedEIP {
			e2e.Logf("Found node %v host egressip %v ", v.Name, expectedEIP)
...
func CreateSCTPclientOnNode(oc *exutil.CLI, pod_pmtrs map[string]string) (err error) {
	...
	defer func() { _ = os.Remove(podFileName) }()
	_ = os.WriteFile(podFileName, []byte(podGenericYaml), 0644)
...
func ConfigIPSecAtRuntime(oc *exutil.CLI, targetStatus string) (err error) {
	...
	if currentStatus == targetStatus {
		...
		o.Eventually(func() error {
			...
			return err
		}, "300s", "30s").ShouldNot(o.Succeed(), "MC applying didn't start yet.")
...
func CheckIPSecInDB(oc *exutil.CLI, targetConfig string) error {
	...
	return wait.PollUntilContextTimeout(context.TODO(), 10*time.Second, 60*time.Second, true, func(_ context.Context) (bool, error) {
...
// CheckSCTPResultPASS
func CheckSCTPResultPASS(oc *exutil.CLI, _, sctpServerPodName, sctpClientPodname, dstIP, dstPort string) {
...
func CheckOVNKState(oc *exutil.CLI) error {
	// check all OVNK pods
	_ = WaitForPodWithLabelReady(oc, "openshift-ovn-kubernetes", "app=ovnkube-node")
	if !IsHypershiftHostedCluster(oc) {
		_ = WaitForPodWithLabelReady(oc, "openshift-ovn-kubernetes", "app=ovnkube-control-plane")
	}
	// check ovnkube-node ds rollout status and confirm if rollout has triggered
	return wait.PollUntilContextTimeout(context.TODO(), 10*time.Second, 2*time.Minute, true, func(_ context.Context) (bool, error) {
...
func GetlrPolicyList(oc *exutil.CLI, nodeName, tableID string, expected bool) ([]string, error) {
	...
	checkLspErr := wait.PollUntilContextTimeout(context.TODO(), 10*time.Second, 2*time.Minute, true, func(_ context.Context) (bool, error) {
...
func GetLBListEndpointsbySVCIPPortinNBDB(oc *exutil.CLI, nodeName, svcPort string) ([]string, error) {
	...
	checkOVNDbErr := wait.PollUntilContextTimeout(context.TODO(), 2*time.Second, 2*time.Minute, true, func(_ context.Context) (bool, error) {
...
func GetAllPodsWithLabelAndCertainState(oc *exutil.CLI, namespace string, label string, podState string) []string {
	...
	o.Expect(allPodsWithLabel).ShouldNot(o.BeEmpty())
...
func GetOVNK8sNodeMgmtIPv6(oc *exutil.CLI, nodeName string) string {
	var cmdOutput string
	var err error
	checkErr := wait.PollUntilContextTimeout(context.TODO(), 2*time.Second, 10*time.Second, true, func(_ context.Context) (bool, error) {
...
func GetJoinSwitchIPofNode(oc *exutil.CLI, nodeName string) ([]string, []string) {
	...
	checkOVNDbErr := wait.PollUntilContextTimeout(context.TODO(), 3*time.Second, 2*time.Minute, true, func(_ context.Context) (bool, error) {
		cmdOutput, cmdErr = RemoteShPodWithBashSpecifyContainer(oc, "openshift-ovn-kubernetes", ovnKubePod, "northd", cmd)
...
func VerifySctpConnPod2IP(oc *exutil.CLI, namespace, sctpServerPodIP, sctpServerPodName, sctpClientPodname string, pass bool) {
	...
	if strings.Contains(msg, "/usr/bin/ncat -l 30102 --sctp") {
		...
		defer func() { _ = cmdNcat.Process.Kill() }()
	...
	_, _ = e2eoutput.RunHostCmd(namespace, sctpClientPodname, "echo 'Test traffic using sctp port from sctpclient to sctpserver' | { ncat -v "+sctpServerPodIP+" 30102 --sctp; }")
...
func GetVIPOnCluster(oc *exutil.CLI, platform string, vipType string) []string {
	...
	checkErr := wait.PollUntilContextTimeout(context.TODO(), 2*time.Second, 10*time.Second, false, func(_ context.Context) (bool, error) {
...
func GetIPv4AndIPWithPrefixForNICOnNode(oc *exutil.CLI, node, nic string) (string, string) {
	...
	o.Expect(len(matches)).Should(o.BeNumerically(">", 1))
...
func GetLBSVCHostname(oc *exutil.CLI, namespace, svc string) string {
	...
	getLBSVCHostnameErr := wait.PollUntilContextTimeout(context.TODO(), 5*time.Second, 2*time.Minute, true, func(_ context.Context) (bool, error) {
...
func GetLBSVCIP(oc *exutil.CLI, namespace string, svcName string) string {
	...
	var cmdErr error
	checkErr := wait.PollUntilContextTimeout(context.TODO(), 5*time.Second, 300*time.Second, true, func(_ context.Context) (bool, error) {
...
func CurlPod2HostPass(_ *exutil.CLI, namespaceSrc, podNameSrc, hostip, DstHostPort string) {
...
func CurlPod2HostFail(_ *exutil.CLI, namespaceSrc, podNameSrc, hostip, DstHostPort string) {
	_, err := e2eoutput.RunHostCmd(namespaceSrc, podNameSrc, "curl -I --connect-timeout 5 -s "+net.JoinHostPort(hostip, DstHostPort))
	o.Expect(err).To(o.HaveOccurred())
}
...
func GetTcpdumpOnNodeCmdFromPod(oc *exutil.CLI, nodeName, tcpdumpCmd, namespace, podname, cmdOnPod string) string {
	...
	defer func() { _ = cmdTcpdump.Process.Kill() }()
	o.Expect(err).NotTo(o.HaveOccurred())
	...
	checkErr := wait.PollUntilContextTimeout(context.Background(), 10*time.Second, 60*time.Second, false, func(_ context.Context) (bool, error) {
...
func WaitForPodsCount(oc *exutil.CLI, namespace, labelSelector string, expectedCount int, interval, timeout time.Duration) error {
	return wait.PollUntilContextTimeout(context.TODO(), interval, timeout, true, func(_ context.Context) (bool, error) {
...
func VerifyIPSecLoaded(oc *exutil.CLI, nodeName string, num int) {
	...
	checkErr := wait.PollUntilContextTimeout(context.Background(), 10*time.Second, 120*time.Second, false, func(_ context.Context) (bool, error) {
...
func GetNodeMTU(oc *exutil.CLI, nodeName string) int {
	...
	o.Expect(len(match)).Should(o.BeNumerically(">", 1))
...
func GetmcpStatus(oc *exutil.CLI, nodeRole string) error {
	return wait.PollUntilContextTimeout(context.TODO(), 60*time.Second, 15*time.Minute, true, func(_ context.Context) (bool, error) {
...
func CheckCNORenderState(oc *exutil.CLI) {
	errCheck := wait.PollUntilContextTimeout(context.Background(), 10*time.Second, 180*time.Second, false, func(_ context.Context) (bool, error) {
...
func VerifyIPSecLoadedInContainers(oc *exutil.CLI, num int) {
	...
	o.Expect(ipsecPods).ShouldNot(o.BeEmpty())
...
func CreateImageDigestMirrorSet(oc *exutil.CLI, imagedigestmirrorsetname string, imageDigestMirrorSetFile string) error {
	...
	return wait.PollUntilContextTimeout(context.TODO(), pollInterval, waitTimeout, true, func(_ context.Context) (bool, error) {
...
func GetSAToken(oc *exutil.CLI) (string, error) {
	return oc.AsAdmin().WithoutNamespace().Run("create").Args("token", "prometheus-k8s", "-n", "openshift-monitoring").Output()
}
...
func AssertOrCheckMCP(oc *exutil.CLI, pool string, interval time.Duration, timeout time.Duration, _ bool) error {
	return wait.PollUntilContextTimeout(context.TODO(), interval, timeout, true, func(_ context.Context) (bool, error) {
