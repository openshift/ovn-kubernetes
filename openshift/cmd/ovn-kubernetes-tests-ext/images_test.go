...
func TestSplitImagePullSpec(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantErrSubstring: "registry host must not include a port",
		},
		{
		...
		},
		{
			...
			wantErrSubstring: "registry host must be a DNS name, not an IP address or localhost",
		},
		{
...
func TestRegisterTestImages(t *testing.T) {
	images.Add(
		"quay.io/sronanrh/iperf:latest",
		"ghcr.io/nicolaka/netshoot:v0.13",
		"docker.io/library/nginx:1",
	)
	required := images.Required()
	if len(required) == 0 {
		t.Fatal("images.Required() returned no images")
	}

	ext := extension.NewExtension("openshift", "payload", "ovn-kubernetes")
	registerTestImages(ext)

	if got, want := len(ext.Images), len(required); got != want {
		t.Fatalf("registered %d images, want %d from images.Required()", got, want)
	}

	for i, img := range ext.Images {
		got := fmt.Sprintf("%s/%s:%s", img.Registry, img.Name, img.Version)
		if got != required[i] {
			t.Errorf("image[%d]: got %q, want %q", i, got, required[i])
		}
	}
}
