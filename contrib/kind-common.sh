...
install_frr_k8s() {
  ...
  # The BGP lanes run two FRR containers:
  ...
  #    The replacement below patches that image to FRR_K8S_FRR_IMAGE.
  #
  # In regular PR e2e lanes where nobody sets a custom FRR_K8S_FRR_IMAGE
  # environment variable, both image variables resolve to FRR_DEPLOYED_IMAGE,
  # so both containers use the same FRR build. FRR_K8S_FRR_IMAGE remains
  # overrideable for tests that intentionally need a different in-cluster
  # daemonset image.
  #
  # Match the manifest's current upstream image exactly. If the pinned frr-k8s
  # manifest changes its FRR image, replace_in_file_or_exit will fail and force
  # this override to be reviewed.
  replace_in_file_or_exit \
    "${FRR_TMP_DIR}"/frr-k8s/config/all-in-one/frr-k8s.yaml \
