...
func (r *ginkgoTestRenamer) generateRename(name string, node types.TestSpec) {
	...
	for {
		...
		for _, label := range r.allLabels {
			...
			if strings.HasPrefix(label, "[ovn-") && strings.Contains(newName, "[ovn-") {
				continue
			}
	...
	if isGoModulePath(codeLocations[len(codeLocations)-1].FileName, "github.com/openshift/ovn-kubernetes", "test/e2e") {
		newLabels += "[Suite:ovn-kubernetes]"
	}
