...
`)
		...
		framework.ExpectNoError(err, "failed to get ovn-kubernetes namespace")
		dbPods, err := e2ekubectl.RunKubectl(ovnKubeNs, "get", "pods", "-l", "app=ovnkube-db", "-o=jsonpath='{.items..metadata.name}'")
		dbContainerName := "nb-ovsdb"
		if isInterconnectEnabled() {
			dbPods, err = e2ekubectl.RunKubectl(ovnKubeNs, "get", "pods", "-l", "app=ovnkube-node", "--field-selector", fmt.Sprintf("spec.nodeName=%s", egress1Node.name), "-o=jsonpath='{.items..metadata.name}'")
		}
...
`)
		...
		if isInterconnectEnabled() {
			dbPods, err = e2ekubectl.RunKubectl(ovnKubeNs, "get", "pods", "-l", "app=ovnkube-node", "--field-selector", fmt.Sprintf("spec.nodeName=%s", egress2Node.name), "-o=jsonpath='{.items..metadata.name}'")
		}
		if err != nil || len(dbPods) == 0 {
			framework.Failf("Error: Check the OVN DB to ensure no SNATs are added for the standby egressIP, err: %v", err)
		}
