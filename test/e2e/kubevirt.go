...
var _ = FDescribe("Kubevirt Virtual Machines", func() {
	...
	type liveMigrationTestData struct {
		...
		numberOfVMs, numberOfMigrations uint
	...
	var (
		...
		checkConnectivity = func(td liveMigrationTestData, vmName string, endpoints []string, stage string) {
		...
		liveMigrateVirtualMachine = func(vmName string, migrationMode kubevirtv1.MigrationMode) {
			...
			Expect(kvcli.VirtualMachine(namespace).Migrate(vmName, &kubevirtv1.MigrateOptions{})).WithOffset(1).To(Succeed())
...
`
		...
		composeVMs = func(numberOfVMs uint, labels map[string]string) ([]*kubevirtv1.VirtualMachine, error) {
		...
		runTest = func(td liveMigrationTestData, vm *kubevirtv1.VirtualMachine) {
			...
			checkConnectivity(td, vm.Name, endpoints, "before live migration")

			for i := uint(1); i <= td.numberOfMigrations; i++ {

				by(vm.Name, fmt.Sprintf("Live migrate virtual machine #%d", i))
				liveMigrateVirtualMachine(vm.Name, td.mode)

				checkConnectivity(td, vm.Name, endpoints, fmt.Sprintf("after live migration #%d", i))
			}
