...
`
	...
	DescribeTable("when live migrated", func(migrationMode kvv1.MigrationMode) {
		...
		By("Wait for virtual machine to receive an address from DHCP")
		Eventually(func() error {
			output, err := kubevirt.RunCommand(kvcli, vmi, "ip a show enp1s0|grep 'inet 10'", 2*time.Second)
			if err != nil {
				return fmt.Errorf("%s: %v", output, err)
			}
			if isDualStack {
				output, err := kubevirt.RunCommand(kvcli, vmi, "ip a show enp1s0|grep 'inet6 fd00'", 2*time.Second)
				if err != nil {
					return fmt.Errorf("%s: %v", output, err)
				}
			}
