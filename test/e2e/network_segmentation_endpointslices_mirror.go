			...
			func(createNetworkFn func(c networkAttachmentConfigParams) error) {
				DescribeTable(
					...
					func(
						...
						deployment := e2edeployment.NewDeployment("test-deployment", replicas, map[string]string{"app": "test"}, "agnhost", deploymentconfig.Get().GetImage(images.Agnhost), appsv1.RollingUpdateDeploymentStrategyType)
