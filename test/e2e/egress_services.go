...
`
			...
			if protocol == v1.IPv6Protocol {
				otherDstIP, err = ipalloc.NewPrimaryIPv6(nodeEgressSubnetCIDR(&dstNode, true))
			} else {
				otherDstIP, err = ipalloc.NewPrimaryIPv4(nodeEgressSubnetCIDR(&dstNode, false))
			}
    ...
    matchLabels:
      kubernetes.io/hostname: ` + nodes[1].Name
			...
			if IsIPv6Cluster(f.ClientSet) {
				egressIP, err = ipalloc.NewPrimaryIPv6(nodeEgressSubnetCIDR(&eipNode, true))
			} else {
				egressIP, err = ipalloc.NewPrimaryIPv4(nodeEgressSubnetCIDR(&eipNode, false))
			}
