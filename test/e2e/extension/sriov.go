...
var _ = g.Describe("[OTP][sig-networking] SDN sriov", func() {
	...
	g.Context("NetObserv SRIOV Scenarios", func() {
		var (
			...
			NOcatSrc            = netobserv.Resource{Kind: "catsrc", Name: "netobserv-konflux-fbc", Namespace: netobservNS}
			subscriptionDir     = testdata.FixturePath("netobserv", "subscription")
			NOSource            = netobserv.CatalogSourceObjects{Channel: "stable", SourceName: NOcatSrc.Name, SourceNamespace: NOcatSrc.Namespace}
			...
			baseDir                  = testdata.FixturePath("netobserv")
		...
		g.BeforeEach(func() {
			...
			if catalogSource != "" {
				...
				compat_otp.ApplyNsResourceFromTemplate(oc, NOcatSrc.Namespace, "-f", catSrcTemplate, "-p", "NAMESPACE="+NOcatSrc.Namespace, "IMAGE="+catalogSource)
			} else {
				e2e.Logf("Using default ystream catalog")
				compat_otp.ApplyNsResourceFromTemplate(oc, NOcatSrc.Namespace, "-f", catSrcTemplate, "-p", "NAMESPACE="+NOcatSrc.Namespace)
			}
			...
			NOexisting, err := netobserv.CheckOperatorStatus(oc, NO.Namespace, NO.PackageName)
			o.Expect(err).NotTo(o.HaveOccurred())
			...
			if !NOexisting {
				...
				NOStatus, err := netobserv.CheckOperatorStatus(oc, NO.Namespace, NO.PackageName)
				o.Expect(err).NotTo(o.HaveOccurred())
				o.Expect((NOStatus)).To(o.BeTrue())
			}
			g.By("Deploy 0-click loki")
			_, _, err = compat_otp.SetupK8SNFSServerAndVolume(oc, 1)
			o.Expect(err).NotTo(o.HaveOccurred())
			compat_otp.ApplyNsResourceFromTemplate(oc, oc.Namespace(), "--ignore-unknown-parameters=true", "-f", lokipvcFixturePath, "-p", "NAMESPACE="+oc.Namespace())
			compat_otp.ApplyNsResourceFromTemplate(oc, oc.Namespace(), "--ignore-unknown-parameters=true", "-f", zeroClickLokiFixturePath)
		...
		g.When("Agents are up prior to SRIOV interfaces", func() {
			g.It("Author:memodi-NonPreRelease-Medium-67619-Verify NetObserv flows are seen on SRIOV interfaces [Serial]", func() {
				...
				compat_otp.ApplyNsResourceFromTemplate(oc, testPod1.namespace, "--ignore-unknown-parameters=true", "-f", testPod1.tempfile, "-p", "PODNAME="+testPod1.name, "SRIOVNETNAME="+testPod1.intfresource, "PING_IP="+testPod1.pingip)
				testPod1.waitForPodReady(oc)
				intfInfo1 := testPod1.getSriovIntfonPod(oc)
				o.Expect(intfInfo1).Should(o.MatchRegexp(testPod1.intfname))
				o.Expect(intfInfo1).Should(o.MatchRegexp(testPod1.ipv4addr))
				e2e.Logf("Check pod %s sriov interface and ip address PASS.", testPod1.name)
				defer testPod2.deletePod(oc)
				compat_otp.ApplyNsResourceFromTemplate(oc, testPod2.namespace, "--ignore-unknown-parameters=true", "-f", testPod2.tempfile, "-p", "PODNAME="+testPod2.name, "SRIOVNETNAME="+testPod2.intfresource, "PING_IP="+testPod2.pingip)
				testPod2.waitForPodReady(oc)
				intfInfo2 := testPod2.getSriovIntfonPod(oc)
				o.Expect(intfInfo2).Should(o.MatchRegexp(testPod2.intfname))
				o.Expect(intfInfo2).Should(o.MatchRegexp(testPod2.ipv4addr))
				...
				// verify flows are enriched for secondary networks
				lokilabels = netobserv.Lokilabels{
					App:              "netobserv-flowcollector",
					SrcK8S_Namespace: testPod1.namespace,
					DstK8S_Namespace: testPod2.namespace,
				}
				flowRecords, err = lokilabels.GetMonolithicLokiFlowLogs("http://localhost:3100", time.Now(), parameters...)
				o.Expect(err).NotTo(o.HaveOccurred())
				o.Expect(len(flowRecords)).To(o.BeNumerically(">", 0), "expected number of enrichedflowRecords to be greater than 0")
			})
		})
		g.When("SRIOV interfaces are up prior to agents", func() {
			g.It("Author:memodi-NonPreRelease-Medium-67619-Verify NetObserv flows are seen on SRIOV interfaces [Serial]", func() {
				...
				flowRecords, err = lokilabels.GetMonolithicLokiFlowLogs("http://localhost:3100", time.Now(), parameters...)
				o.Expect(err).NotTo(o.HaveOccurred())
				o.Expect(len(flowRecords)).To(o.BeNumerically(">", 0), "expected number of enriched flowRecords to be greater than 0")
			})
		})
	})
})
